...
_FP_DEFAULTS = {
    ...
    "curr_hsa": 0.0,
    # --- Growing: Concentrated Stock ---
    "conc_stock_value": 0.0, "conc_stock_basis": 0.0, "conc_stock_label": "",
    "conc_stock_div_yield": 0.0,
    "conc_stock_unwind": False, "conc_stock_unwind_years": 0,
    ...
    "aa_life_eq": 40, "aa_life_intl": 10, "aa_life_fi": 40, "aa_life_re": 10,
    ...
    "vt_taxable": "individual", "vt_annuity": "mutual_funds", "vt_life": "mutual_funds",
    ...
    "wages": 0.0, "tax_exempt_interest": 0.0, "other_income": 0.0, "other_income_tax_free": False,
    "wages_start_age": 0, "wages_end_age": 0,
    "other_income_start_age": 0, "other_income_end_age": 0, "other_income_inflate": True,
    ...
    "charitable_bequest_mode": "pct",   # "pct" or "fixed"
    "charitable_bequest_pct": 0.0,      # percentage of gross estate
    "charitable_bequest_fixed": 0.0,    # fixed dollar amount
    "charitable_bequest_inflate": False, # inflate fixed bequest with CPI
    "stock_donation_annual": 0.0,       # annual appreciated stock gift from brokerage
    "stock_donation_end_age": 70,        # stop donating stock at this age
    "stock_donation_additional": False,  # True = extra giving; False = replaces cash giving
...
# Restore D from pre-aggregation snapshot (undo in-place aggregation from last rerun)
_d_snapshot = st.session_state.pop("_d_pre_aggregate", None)
if _d_snapshot:
    D.update(_d_snapshot)
...
_COMP_DEFAULTS = {
    ...
    "gross_from_needs": 0,
    "tab3_rows": None, "tab3_params": None, "tab3_mc": None, "tab3_tax_results": [],
    # Unified optimizer
    ...
    "opt_plus_results": None, "opt_plus_scenarios": None,
    ...
    "_preret_mc": None, "_preret_opt": None, "_preret_sweep": None, "_preret_opt_mc": None,
}
for _k, _v in _COMP_DEFAULTS.items():
    if _k not in st.session_state:
        st.session_state[_k] = _v
# User-entered lists — NOT in _COMP_DEFAULTS so profile load doesn't wipe them
for _k, _v in {"additional_expenses": [], "future_income": [], "accounts": []}.items():
    if _k not in st.session_state:
        st.session_state[_k] = _v
if "_acct_next_id" not in st.session_state:
    st.session_state._acct_next_id = 1
...
# ══════════════════════════════════════════════════════════════════════
# ADDITIONAL ACCOUNTS — dynamic "Add Account" support
# ══════════════════════════════════════════════════════════════════════
_ADDL_ACCOUNT_TYPES = [
    "401(k)", "Traditional IRA", "Roth IRA", "Roth 401(k)",
    "Taxable Brokerage", "Cash", "HSA",
    "Annuity", "Life Insurance", "Concentrated Stock", "Inherited IRA",
]
_SINGLETON_ACCT_TYPES = {"Life Insurance", "Concentrated Stock", "Cash", "HSA"}

def _make_default_account(acct_type, next_id):
    acct = {"id": f"acct_{next_id}", "type": acct_type,
            "owner": "Filer", "label": "", "balance": 0.0}
    if acct_type == "Taxable Brokerage":
        acct["basis"] = 0.0
    elif acct_type == "Annuity":
        acct["basis"] = 0.0
        acct["monthly_contribution"] = 0.0
    elif acct_type == "Life Insurance":
        acct.update({"basis": 0.0, "death_benefit": 0.0, "monthly_premium": 0.0,
                     "payout": "Last to Die", "db_grows": False,
                     "spend_cv": False, "pay_to_age": 100.0})
    elif acct_type == "Concentrated Stock":
        acct.update({"basis": 0.0, "div_yield": 0.0, "unwind": False, "unwind_years": 0})
    elif acct_type == "Inherited IRA":
        acct.update({"rule": "10-Year Rule", "years_remaining": 10,
                     "rmd_required": False, "additional_distribution": 0.0})
    return acct

_ACCT_AGGREGATED_KEYS = [
    "curr_401k_f", "curr_401k_s", "curr_trad_ira_f", "curr_trad_ira_s",
    "curr_roth_ira_f", "curr_roth_ira_s", "curr_roth_401k",
    "curr_taxable", "taxable_basis", "curr_taxable_s", "taxable_basis_s",
    "curr_cash", "curr_hsa",
    "annuity_value_f", "annuity_basis_f", "annuity_monthly_f",
    "annuity_value_s", "annuity_basis_s", "annuity_monthly_s",
    "life_cash_value", "life_basis", "life_monthly", "life_death_benefit", "life_pay_to_age",
    "life_payout", "life_db_grows", "life_spend_cv",
    "conc_stock_value", "conc_stock_basis", "conc_stock_div_yield", "conc_stock_unwind_years",
    "conc_stock_label", "conc_stock_unwind",
    "inh_ira_f", "inh_add_f", "inh_ira_s", "inh_add_s",
    "inh_rule_f", "inh_yrs_f", "inh_rmd_req_f",
    "inh_rule_s", "inh_yrs_s", "inh_rmd_req_s",
]

def _aggregate_additional_accounts():
    """Sum dynamic account entries into flat D keys (additive to static fields).

    D is saved before this call and restored at the top of the next rerun,
    so the in-place modifications here are temporary (computation only).
    """
    for acct in st.session_state.get("accounts", []):
        t = acct["type"]
        owner = acct.get("owner", "Filer")
        bal = float(acct.get("balance", 0))
        is_filer = (owner == "Filer")
        if bal == 0 and t not in ("Life Insurance", "Concentrated Stock"):
            continue

        if t == "401(k)":
            D["curr_401k_f" if is_filer else "curr_401k_s"] += bal
        elif t == "Traditional IRA":
            D["curr_trad_ira_f" if is_filer else "curr_trad_ira_s"] += bal
        elif t == "Roth IRA":
            D["curr_roth_ira_f" if is_filer else "curr_roth_ira_s"] += bal
        elif t == "Roth 401(k)":
            D["curr_roth_401k"] += bal
        elif t == "Taxable Brokerage":
            if is_filer:
                D["curr_taxable"] += bal
                D["taxable_basis"] += float(acct.get("basis", 0))
            else:
                D["curr_taxable_s"] += bal
                D["taxable_basis_s"] += float(acct.get("basis", 0))
        elif t == "Cash":
            D["curr_cash"] += bal
        elif t == "HSA":
            D["curr_hsa"] += bal
            ...
            if is_filer:
                D["annuity_value_f"] += bal
                D["annuity_basis_f"] += float(acct.get("basis", 0))
                D["annuity_monthly_f"] += float(acct.get("monthly_contribution", 0))
            else:
                D["annuity_value_s"] += bal
                D["annuity_basis_s"] += float(acct.get("basis", 0))
                D["annuity_monthly_s"] += float(acct.get("monthly_contribution", 0))
        elif t == "Life Insurance":
            D["life_cash_value"] += bal
            D["life_basis"] += float(acct.get("basis", 0))
            D["life_monthly"] += float(acct.get("monthly_premium", 0))
            D["life_death_benefit"] += float(acct.get("death_benefit", 0))
            D["life_payout"] = acct.get("payout", D["life_payout"])
            D["life_db_grows"] = bool(acct.get("db_grows", D["life_db_grows"]))
            D["life_spend_cv"] = bool(acct.get("spend_cv", D["life_spend_cv"]))
            D["life_pay_to_age"] = float(acct.get("pay_to_age", D["life_pay_to_age"]))
        elif t == "Concentrated Stock":
            D["conc_stock_value"] += bal
            D["conc_stock_basis"] += float(acct.get("basis", 0))
            D["conc_stock_label"] = acct.get("label", "") or D["conc_stock_label"]
            D["conc_stock_div_yield"] = float(acct.get("div_yield", 0)) or D["conc_stock_div_yield"]
            D["conc_stock_unwind"] = bool(acct.get("unwind", False)) or D["conc_stock_unwind"]
            D["conc_stock_unwind_years"] = int(acct.get("unwind_years", 0)) or D["conc_stock_unwind_years"]
        elif t == "Inherited IRA":
            suffix = "f" if is_filer else "s"
            D[f"inh_ira_{suffix}"] += bal
            D[f"inh_rule_{suffix}"] = acct.get("rule", D[f"inh_rule_{suffix}"])
            D[f"inh_yrs_{suffix}"] = int(acct.get("years_remaining", D[f"inh_yrs_{suffix}"]))
            D[f"inh_rmd_req_{suffix}"] = bool(acct.get("rmd_required", D[f"inh_rmd_req_{suffix}"]))
            D[f"inh_add_{suffix}"] += float(acct.get("additional_distribution", 0))

def _sync_accounts_from_widgets():
    """Read account widget keys back into st.session_state.accounts."""
    for idx, acct in enumerate(st.session_state.get("accounts", [])):
        for wk, ak in [("bal", "balance"), ("own", "owner"), ("lbl", "label"),
                        ("basis", "basis"), ("mc", "monthly_contribution"),
                        ("mp", "monthly_premium"), ("db", "death_benefit"),
                        ("pta", "pay_to_age"), ("dy", "div_yield"),
                        ("uwy", "unwind_years"), ("addl", "additional_distribution"),
                        ("yrs", "years_remaining")]:
            _sk = f"_acct_{wk}_{idx}"
            if _sk in st.session_state:
                acct[ak] = st.session_state[_sk]
        for wk, ak in [("dbg", "db_grows"), ("scv", "spend_cv"),
                        ("uw", "unwind"), ("rmd", "rmd_required")]:
            _sk = f"_acct_{wk}_{idx}"
            if _sk in st.session_state:
                acct[ak] = bool(st.session_state[_sk])
        for wk, ak in [("po", "payout"), ("rule", "rule")]:
            _sk = f"_acct_{wk}_{idx}"
            if _sk in st.session_state:
                acct[ak] = st.session_state[_sk]
...
def _sync_lists_from_widgets():
    """Sync additional_expenses and future_income from widget keys.

    The UI code that assigns widget values back into the list dicts only runs
    when the corresponding nav tab is active.  This function reads the widget
    keys directly so Save always captures the latest values.

    Tab 2 ("Receiving"/"Spending") stores start/end as calendar years in widgets;
    Tab 3 ("Achieving") stores them as ages.  We convert years→ages for Tab 2.
    """
    _dob = D.get("filer_dob")
    _age_now = TEA.age_at_date(_dob, dt.date.today()) if _dob else 70
    _birth_year = dt.date.today().year - _age_now

    for idx, ae in enumerate(st.session_state.get("additional_expenses", [])):
        # Tab 2 keys: prefix "_ae_" (start/end are years)
        if f"_ae_name_{idx}" in st.session_state:
            ae["name"] = st.session_state[f"_ae_name_{idx}"]
            ae["net_amount"] = st.session_state[f"_ae_amt_{idx}"]
            ae["inflates"] = st.session_state[f"_ae_inf_{idx}"]
            ae["source"] = st.session_state[f"_ae_src_{idx}"]
            if f"_ae_start_{idx}" in st.session_state:
                ae["start_age"] = st.session_state[f"_ae_start_{idx}"] - _birth_year
            if f"_ae_end_{idx}" in st.session_state:
                _end_yr = st.session_state[f"_ae_end_{idx}"]
                ae["end_age"] = (_end_yr - _birth_year) if _end_yr > 0 else 0
        # Tab 3 keys: prefix "fp_ae_" (start/end are ages)
        if f"fp_ae_name_{idx}" in st.session_state:
            ae["name"] = st.session_state[f"fp_ae_name_{idx}"]
            ae["net_amount"] = st.session_state[f"fp_ae_amt_{idx}"]
            ae["inflates"] = st.session_state[f"fp_ae_inf_{idx}"]
            ae["source"] = st.session_state[f"fp_ae_src_{idx}"]
            if f"fp_ae_start_{idx}" in st.session_state:
                ae["start_age"] = st.session_state[f"fp_ae_start_{idx}"]
            if f"fp_ae_end_{idx}" in st.session_state:
                ae["end_age"] = st.session_state[f"fp_ae_end_{idx}"]

    for idx, fi in enumerate(st.session_state.get("future_income", [])):
        # Tab 2 keys: prefix "_fi_" (start/end are years)
        if f"_fi_name_{idx}" in st.session_state:
            fi["name"] = st.session_state[f"_fi_name_{idx}"]
            fi["amount"] = st.session_state[f"_fi_amt_{idx}"]
            fi["tax_type"] = st.session_state[f"_fi_tax_{idx}"]
            if f"_fi_inf_{idx}" in st.session_state:
                fi["inflates"] = st.session_state[f"_fi_inf_{idx}"]
            if f"_fi_start_{idx}" in st.session_state:
                fi["start_age"] = st.session_state[f"_fi_start_{idx}"] - _birth_year
            if f"_fi_end_{idx}" in st.session_state:
                _end_yr = st.session_state[f"_fi_end_{idx}"]
                fi["end_age"] = (_end_yr - _birth_year) if _end_yr > 0 else 0
            if f"_fi_basis_{idx}" in st.session_state:
                fi["basis"] = st.session_state[f"_fi_basis_{idx}"]
        # Tab 3 keys: prefix "fp_fi_" (start/end are ages)
        if f"fp_fi_name_{idx}" in st.session_state:
            fi["name"] = st.session_state[f"fp_fi_name_{idx}"]
            fi["amount"] = st.session_state[f"fp_fi_amt_{idx}"]
            fi["tax_type"] = st.session_state[f"fp_fi_tax_{idx}"]
            if f"fp_fi_inf_{idx}" in st.session_state:
                fi["inflates"] = st.session_state[f"fp_fi_inf_{idx}"]
            if f"fp_fi_start_{idx}" in st.session_state:
                fi["start_age"] = st.session_state[f"fp_fi_start_{idx}"]
            if f"fp_fi_end_{idx}" in st.session_state:
                fi["end_age"] = st.session_state[f"fp_fi_end_{idx}"]
            if f"fp_fi_basis_{idx}" in st.session_state:
                fi["basis"] = st.session_state[f"fp_fi_basis_{idx}"]

def _save_profile(name):
    name = name.strip()
    _sync_lists_from_widgets()
    _sync_accounts_from_widgets()
    data = {}
    for k, v in D.items():
    ...
    data["_accounts"] = st.session_state.accounts
...
def _load_profile(name):
    ...
    with open(path) as f:
        data = json.load(f)
    # Reset all D keys to defaults before loading to prevent cross-client leakage
    ...
    # Migrate old conc_stock_unwind_by_age → conc_stock_unwind_years
    if "conc_stock_unwind_by_age" in data and "conc_stock_unwind_years" not in data:
        data["conc_stock_unwind_years"] = data["conc_stock_unwind_by_age"]
    ...
    if "_future_income" in data:
        for _fi_item in data["_future_income"]:
            if "taxable" in _fi_item and "tax_type" not in _fi_item:
                _fi_item["tax_type"] = "Ordinary Income" if _fi_item["taxable"] else "Non-Taxable"
            _fi_item.setdefault("tax_type", "Ordinary Income")
            _fi_item.setdefault("basis", 0.0)
        st.session_state.future_income = data["_future_income"]
    if "_accounts" in data:
        st.session_state.accounts = data["_accounts"]
        st.session_state._acct_next_id = max((int(a["id"].split("_")[1]) for a in data["_accounts"]), default=0) + 1
    else:
    ...
    for wk in list(st.session_state.keys()):
        ...
        elif wk.startswith("_acct_") and wk != "_acct_next_id":
            del st.session_state[wk]
        elif wk.startswith("_sb_") and wk not in _keep:
            del st.session_state[wk]
        elif wk.startswith("fp_") and wk not in _keep:
            del st.session_state[wk]
    # Queue client name update for next rerun (can't set after widget instantiated)
    st.session_state["_pending_client_name"] = D["client_name"]
...
def _load_tax_profile(name):
    ...
    _direct = [
        ...
        "cap_gain_loss", "other_income", "other_income_tax_free",
        ...
        "charitable_bequest_mode", "charitable_bequest_pct", "charitable_bequest_fixed", "charitable_bequest_inflate",
        "stock_donation_annual", "stock_donation_end_age", "stock_donation_additional",
    ...
    # Tax profiles don't have additional accounts
    st.session_state.accounts = []
    st.session_state._acct_next_id = 1
...
def _on_new_profile():
    ...
    for ck in list(_COMP_DEFAULTS.keys()):
        st.session_state[ck] = _COMP_DEFAULTS[ck]
    for wk in list(st.session_state.keys()):
        if wk.startswith(("_w_", "_sb_", "_ae_", "_acct_", "fp_ae_", "fp_rr_", "fp_preret_", "fp_opt_", "fp_")):
            if wk not in {"_btn_save", "_btn_load", "_btn_new", "_sb_load_sel", "fp_nav", "fp_data", "_acct_next_id"}:
                del st.session_state[wk]
    st.session_state["_fp_did_new"] = True

with st.sidebar:
    st.subheader("Profile")
    # Apply pending client name from profile load (must happen before widget renders)
    _pending_name = st.session_state.pop("_pending_client_name", None)
    if _pending_name is not None:
        st.session_state["_sb_client_name"] = _pending_name
...
# ══════════════════════════════════════════════════════════════════════
def _fp_pre_ret_tabs(tab2, tab3, tab4, tab6, D, is_joint, filing_status,
    ...
    # Employee-only savings (excludes employer match — match is "free money", not from paycheck)
    _employer_match_total = employer_match + employer_match_spouse
    _employee_savings = (contrib_dict["pretax"] + contrib_dict["roth"]
                         + contrib_taxable + contrib_hsa - _employer_match_total)
    st.session_state._fp_employee_savings = _employee_savings
    ...
    start_balances = {
        ...
        "hsa": D["curr_hsa"],
        "conc_stock": conc_stock_value,
        "conc_stock_basis": conc_stock_basis,
    }
    ...
    income_info = {
        ...
        "other_income": D["other_income"], "other_income_tax_free": bool(D["other_income_tax_free"]),
        "other_income_inflation": bool(D["other_income_inflate"]),
        "other_income_start_year": max(0, int(D["other_income_start_age"]) - current_age_filer) if int(D["other_income_start_age"]) > 0 else 0,
        "other_income_years": (int(D["other_income_end_age"]) - max(int(D["other_income_start_age"]) or current_age_filer, current_age_filer)) if int(D["other_income_end_age"]) > 0 else 0,
    ...
    # --- Active future income (for cash flow / spending inference) ---
    _active_future_inc = sum(
        fi["amount"] for fi in st.session_state.get("future_income", [])
        if fi.get("amount", 0) > 0
        and fi.get("start_age", 999) <= current_age_filer
        and (fi.get("end_age", 0) == 0 or fi.get("end_age", 0) > current_age_filer))

    # --- Inherited IRA current-year distribution (for spending inference) ---
    # Excluded from "income" for spending calc — it's asset liquidation, not lifestyle income.
    # The TAX on it is still captured (reduces available spending).
    _inh_dist_for_cf = 0.0
    if D["inh_ira_f"] > 0:
        _cf_rule_f = "10_year" if D["inh_rule_f"] == "10-Year Rule" else "lifetime"
        if bool(D["inh_rmd_req_f"]) or _cf_rule_f == "lifetime":
            _cf_div_f = engine.SINGLE_LIFE_TABLE.get(current_age_filer, 1.0)
            _inh_dist_for_cf += D["inh_ira_f"] / _cf_div_f if _cf_div_f > 0 else D["inh_ira_f"]
        _inh_dist_for_cf += float(D["inh_add_f"])
    if is_joint and D["inh_ira_s"] > 0:
        _cf_rule_s = "10_year" if D["inh_rule_s"] == "10-Year Rule" else "lifetime"
        if bool(D["inh_rmd_req_s"]) or _cf_rule_s == "lifetime":
            _cf_age_s = current_age_spouse or current_age_filer
            _cf_div_s = engine.SINGLE_LIFE_TABLE.get(_cf_age_s, 1.0)
            _inh_dist_for_cf += D["inh_ira_s"] / _cf_div_s if _cf_div_s > 0 else D["inh_ira_s"]
        _inh_dist_for_cf += float(D["inh_add_s"])

    # --- Inferred spending (computed early so _build_retire_params can use it) ---
    _si_base_early = st.session_state.base_results
    _si_gross_early = _si_base_early.get("spendable_gross", 0) if _si_base_early else 0.0
    _si_taxes_early = ((_si_base_early["total_tax"] + _si_base_early.get("fica", 0)
                        + _si_base_early.get("medicare_premiums", 0)) if _si_base_early else 0.0)
    _si_savings_early = _employee_savings
    _inferred_spending = max(0, _si_gross_early - _inh_dist_for_cf - _si_taxes_early - _si_savings_early - mtg_payment_annual)
    _use_override = st.session_state.get("fp_spending_override", False)
    _proj_spending = D["living_expenses"] if _use_override else _inferred_spending
    st.session_state._fp_proj_spending = _proj_spending
    # Auto-rerun projections when spending override changes
    _last_proj_spending = st.session_state.get("_fp_last_proj_spending")
    _spending_changed = (_last_proj_spending is not None
                         and abs(_proj_spending - _last_proj_spending) > 1.0
                         and st.session_state.get("_preret_mc") is not None)
    # Sync accumulation engine with chosen spending source
    income_info["living_expenses"] = _proj_spending
    ...
    def _build_retire_params(accum_result):
        ...
        ret_expenses = _proj_spending * (D["ret_pct"] / 100) * inf_factor
        ...
        return {
            ...
            "heir_bracket_option": "same",
            "other_income": (D["other_income"] * (inf_factor if D["other_income_inflate"] else 1.0)
                             if not (_oi_end > 0 and _oi_end <= ret_age) else 0.0)
                            + (_ire_keep_income if not inv_re_properties else 0.0),
            "other_income_tax_free": bool(D["other_income_tax_free"]),
            "other_income_inflation": bool(D["other_income_inflate"]),
            "other_income_start_year": max(0, _oi_start - ret_age) if _oi_start > ret_age else 0,
            "other_income_years": (_oi_end - max(_oi_start or ret_age, ret_age)) if (_oi_end > 0 and _oi_end > ret_age) else 0,
            "inv_re_properties": [{
                "name": p["name"],
            ...
            "life_death_benefit": (float(D["life_death_benefit"]) * ((1 + r_life) ** years_to_ret)
                                   if D["life_db_grows"] else float(D["life_death_benefit"])),
            ...
            "state_estate_exemption": float(D["state_estate_exemption"]),
            # Concentrated stock
            "conc_stock_gain_pct": float(conc_stock_gain_pct),
            "conc_stock_div_yield": float(conc_stock_div_yield),
            "conc_stock_unwind": bool(D["conc_stock_unwind"]),
            "conc_stock_unwind_years": int(D["conc_stock_unwind_years"]),
        }
    ...
    def _conc_at_retirement(yrs):
        """Grow concentrated stock to retirement (basis unchanged, price appreciates)."""
        if conc_stock_value <= 0:
            return 0.0, 0.0
        _gr = (1 + r_taxable - conc_stock_div_yield) ** yrs if yrs > 0 else 1.0
        return conc_stock_value * _gr, conc_stock_basis
    ...
    with tab2:
        ...
        _cur_portfolio = total_pretax + total_roth + taxable_brokerage_bal + taxable_cash_bal + D["curr_hsa"] + conc_stock_value
        _roth_401k_bal = D["curr_roth_401k"]
        _roth_ira_bal = total_roth - _roth_401k_bal
        _ncur = 5
        if _roth_401k_bal > 0: _ncur += 1
        if conc_stock_value > 0: _ncur += 1
        cols = st.columns(_ncur)
        with cols[0]: st.metric("Pre-Tax", TEA.money(total_pretax))
        _ci_b = 1
        if _roth_401k_bal > 0:
            with cols[_ci_b]: st.metric("Roth IRA", TEA.money(_roth_ira_bal))
            _ci_b += 1
            with cols[_ci_b]: st.metric("Roth 401(k)", TEA.money(_roth_401k_bal))
        else:
            with cols[_ci_b]: st.metric("Roth", TEA.money(total_roth))
        _ci_b += 1
        with cols[_ci_b]: st.metric("Taxable", TEA.money(taxable_brokerage_bal + taxable_cash_bal))
        _ci_b += 1
        if conc_stock_value > 0:
            with cols[_ci_b]: st.metric(conc_stock_label, TEA.money(conc_stock_value))
            _ci_b += 1
        with cols[_ci_b]: st.metric("HSA", TEA.money(D["curr_hsa"]))
        with cols[_ci_b + 1]: st.metric("Portfolio", TEA.money(_cur_portfolio))
        ...
        _sav_parts = []
        if contrib_dict["pretax"] > 0: _sav_parts.append(f"Pre-Tax {TEA.money(contrib_dict['pretax'])}")
        _roth_401k_total = contrib_401k_roth_filer + contrib_401k_roth_spouse
        _roth_ira_total = contrib_roth_ira + contrib_roth_ira_spouse
        if _roth_401k_total > 0: _sav_parts.append(f"Roth 401(k) {TEA.money(_roth_401k_total)}")
        if _roth_ira_total > 0: _sav_parts.append(f"Roth IRA {TEA.money(_roth_ira_total)}")
        if contrib_taxable > 0: _sav_parts.append(f"Taxable {TEA.money(contrib_taxable)}")
        if contrib_hsa > 0: _sav_parts.append(f"HSA {TEA.money(contrib_hsa)}")
        _sav_total = _employee_savings
        _sav_match_str = f" + {TEA.money(_employer_match_total)} employer match" if _employer_match_total > 0 else ""
        st.caption(f"Annual savings: {' | '.join(_sav_parts)} | Employee total {TEA.money(_sav_total)}{_sav_match_str}")

        # --- Annual Cash Flow Summary ---
        st.markdown("### Annual Cash Flow Summary")
        _cf_base = st.session_state.base_results
        _cf_income = _cf_base.get("spendable_gross", 0) if _cf_base else 0.0
        ...
        _cf_taxes = (_cf_base["total_tax"] + _cf_base.get("fica", 0) + _cf_base.get("medicare_premiums", 0)) if _cf_base else 0.0
        _cf_savings = _employee_savings
        _cf_surplus = _cf_income - _cf_taxes - _cf_savings - _cf_living - _cf_future_exp
        _cf_ncols = 6 if _cf_future_exp != 0 else 5
        _cf_cols = st.columns(_cf_ncols)
        with _cf_cols[0]: st.metric("Spendable Income", TEA.money(_cf_income))
        with _cf_cols[1]: st.metric("Taxes", TEA.money(_cf_taxes))
        with _cf_cols[2]: st.metric("Total Savings", TEA.money(_cf_savings))
        _ci = 3
        with _cf_cols[_ci]: st.metric("Living + Mortgage", TEA.money(_cf_living))
        _ci += 1
        if _cf_future_exp != 0:
            with _cf_cols[_ci]: st.metric("Other Expenses", TEA.money(_cf_future_exp))
            _ci += 1
        with _cf_cols[_ci]: st.metric("Surplus / (Deficit)", TEA.money(_cf_surplus))
        # Income breakdown (from tax return cashflow)
        _cf_parts = []
        if _cf_base:
            if _cf_base.get("wages", 0) > 0: _cf_parts.append(f"Wages {TEA.money(_cf_base['wages'])}")
            if _cf_base.get("gross_ss", 0) > 0: _cf_parts.append(f"SS {TEA.money(_cf_base['gross_ss'])}")
            if _cf_base.get("taxable_pensions", 0) > 0: _cf_parts.append(f"Pension {TEA.money(_cf_base['taxable_pensions'])}")
            _cf_own_ira = max(0, _cf_base.get("taxable_ira", 0) - _inh_dist_for_cf)
            if _cf_own_ira > 0: _cf_parts.append(f"IRA dist {TEA.money(_cf_own_ira)}")
            if _cf_base.get("other_income", 0) > 0: _cf_parts.append(f"Other {TEA.money(_cf_base['other_income'])}")
            if _cf_base.get("cashflow_taxfree", 0) > 0: _cf_parts.append(f"Tax-free {TEA.money(_cf_base['cashflow_taxfree'])}")
            _inv_inc = (_cf_base.get("total_ordinary_dividends", 0) + _cf_base.get("interest_taxable", 0)
                        + _cf_base.get("tax_exempt_interest", 0) - _cf_base.get("reinvested_amount", 0))
            if _inv_inc > 0: _cf_parts.append(f"Invest. income {TEA.money(_inv_inc)}")
        if _cf_parts:
            st.caption("Income: " + " | ".join(_cf_parts))
        ...
        _run_proj_btn = st.button("Run Projection", type="primary", key="fp_preret_run_proj")
        if _run_proj_btn or _spending_changed:
            accum_result = _run_accum()
            st.session_state.projection_results = accum_result
            st.session_state._fp_last_proj_spending = _proj_spending
            # Run Monte Carlo automatically
            _spouse_age_at_ret = (current_age_spouse + years_to_ret) if current_age_spouse else None
            _ret_yrs_f = filer_plan_through_age - ret_age
            _ret_yrs_s = (spouse_plan_through_age - _spouse_age_at_ret) if (_spouse_age_at_ret and spouse_plan_through_age) else 0
            _ret_years = max(1, max(_ret_yrs_f, _ret_yrs_s) + 1)
            n_total = years_to_ret + 1 + max(1, _ret_years)
            def _mc_run(return_seq):
                ...
                _bals = {
                    ...
                    "conc_stock": _conc_at_retirement(years_to_ret)[0],
                    "conc_stock_basis": _conc_at_retirement(years_to_ret)[1],
                }
            ...
            # --- Age Sweep ---
            _sweep_start = max(current_age_filer + 1, 55)
            _sweep_end = min(current_age_filer + 25, 75)
            if _sweep_start <= _sweep_end:
                def _build_sweep_params(acc_res, sweep_ytr, sweep_ra):
                    """Build balances + retire params for a given retirement age."""
                    _sf = acc_res["rows"][-1] if acc_res["rows"] else {}
                    _sb = acc_res.get("final_brokerage", _sf.get("Bal Taxable", 0))
                    _sc = acc_res.get("final_cash", 0)
                    if _ire_liq_equity > 0:
                        _sb += _ire_liq_equity
                    _sbals = {
                        "pretax": _sf.get("Bal Pre-Tax", 0),
                        "roth": _sf.get("Bal Roth", 0),
                        "brokerage": _sb, "cash": _sc,
                        "taxable": _sb + _sc,
                        "brokerage_basis": acc_res.get("final_basis", 0) + _ire_liq_equity,
                        "hsa": _sf.get("Bal HSA", 0),
                        "conc_stock": _conc_at_retirement(sweep_ytr)[0],
                        "conc_stock_basis": _conc_at_retirement(sweep_ytr)[1],
                    }
                    # Build retire params with adjusted years_to_ret / ret_age
                    _inf_f = (1 + inflation) ** sweep_ytr
                    _pen_f_cola = D["pension_cola_filer"] / 100
                    _pen_s_cola = D["pension_cola_spouse"] / 100
                    _pf = pension_filer * ((1 + _pen_f_cola) ** sweep_ytr) if pension_filer > 0 else 0.0
                    _ps = pension_spouse * ((1 + _pen_s_cola) ** sweep_ytr) if pension_spouse > 0 else 0.0
                    if filer_ss_already and filer_ss_current > 0:
                        _ssf = filer_ss_current * _inf_f
                    else:
                        _ssf = ss_filer_pia * _inf_f
                    if is_joint and spouse_ss_already and spouse_ss_current > 0:
                        _sss = spouse_ss_current * _inf_f
                    else:
                        _sss = ss_spouse_pia * _inf_f
                    _hv = home_value * ((1 + home_appreciation) ** sweep_ytr)
                    _my = max(0, mtg_years - sweep_ytr)
                    _rfe = [dict(fe) for fe in future_expenses if fe["end_age"] > sweep_ra]
                    _iira = acc_res.get("inherited_iras_state", [])
                    _sd = "none" if D["surplus_dest"] == "Don't Reinvest" else ("cash" if D["surplus_dest"] == "Cash/Savings" else "brokerage")
                    _srp = {
                        "retire_age": sweep_ra,
                        "filer_life_expectancy": filer_plan_through_age,
                        "spouse_life_expectancy": spouse_plan_through_age,
                        "survivor_spending_pct": survivor_spending_pct,
                        "pension_survivor_pct": pension_survivor_pct,
                        "retire_year": current_year + sweep_ytr,
                        "inflation": inflation, "bracket_growth": bracket_growth,
                        "medicare_growth": medicare_growth,
                        "post_retire_return": post_ret_return,
                        "r_pretax": r_pretax, "r_roth": r_roth,
                        ...
                        "out_of_state_gain": float(D["out_of_state_gain"]),
                        "base_tax_year": int(D["tax_year"]),
                        "expenses_at_retirement": _proj_spending * (D["ret_pct"] / 100) * _inf_f,
                        "ss_filer_fra": _ssf, "ss_spouse_fra": _sss,
                        "ss_filer_claim_age": ss_filer_claim_age,
                        "ss_spouse_claim_age": ss_spouse_claim_age,
                        "filer_dob": filer_dob,
                        "spouse_dob": spouse_dob if is_joint else None,
                        ...
                        "ss_spouse_already": spouse_ss_already if is_joint else False,
                        "ss_spouse_current_benefit": spouse_ss_current if (is_joint and spouse_ss_already) else 0,
                        "pension_filer_at_retire": _pf,
                        "pension_filer_start_age": int(D["pension_filer_age"]),
                        "pension_filer_cola": _pen_f_cola,
                        "pension_spouse_at_retire": _ps,
                        "pension_spouse_start_age": int(D["pension_spouse_age"]) if is_joint else 65,
                        "pension_spouse_cola": _pen_s_cola,
                        "spouse_age_at_retire": (current_age_spouse + sweep_ytr) if current_age_spouse else None,
                        "mortgage_payment": mtg_payment_annual,
                        "mortgage_years_at_retire": _my,
                        "home_value_at_retire": _hv,
                        "home_appreciation": home_appreciation,
                        "inv_re_value_at_retire": _ire_keep_value,
                        "inv_re_appr": _ire_keep_appr,
                        "future_expenses": _rfe,
                        "dividend_yield": acc_res.get("derived_div_yield", div_yield),
                        "brok_interest_yield": acc_res.get("derived_brok_int_yield", 0.0),
                        "cash_interest_rate": cash_int_rate,
                        "charitable": charitable, "qcd_annual": qcd_annual,
                        "inherited_iras": _iira,
                        "surplus_destination": _sd,
                        "heir_bracket_option": "same",
                        "other_income": (D["other_income"] * (_inf_f if D["other_income_inflate"] else 1.0)
                                         if not (_oi_end > 0 and _oi_end <= sweep_ra) else 0.0)
                                        + (_ire_keep_income if not inv_re_properties else 0.0),
                        "other_income_tax_free": bool(D["other_income_tax_free"]),
                        "other_income_inflation": bool(D["other_income_inflate"]),
                        "other_income_start_year": max(0, _oi_start - sweep_ra) if _oi_start > sweep_ra else 0,
                        "other_income_years": (_oi_end - max(_oi_start or sweep_ra, sweep_ra)) if (_oi_end > 0 and _oi_end > sweep_ra) else 0,
                        "inv_re_properties": [{
                            "name": p["name"],
                            "net_income": p["net_income"] * ((1 + p["income_growth"]) ** sweep_ytr),
                            "income_growth": p["income_growth"],
                            "mortgage_pmt": p["mortgage_pmt"],
                            "mortgage_years": max(0, p["mortgage_years"] - sweep_ytr),
                        } for p in inv_re_properties],
                        "life_cash_value": life_cash_value * ((1 + r_life) ** sweep_ytr),
                        "life_death_benefit": (float(D["life_death_benefit"]) * ((1 + r_life) ** sweep_ytr)
                                               if D["life_db_grows"] else float(D["life_death_benefit"])),
                        "life_db_grows": bool(D["life_db_grows"]),
                        "life_spend_cv": bool(D["life_spend_cv"]),
                        "r_life": r_life,
                        "annuity_value": annuity_value * ((1 + r_annuity) ** sweep_ytr),
                        "annuity_basis": annuity_basis,
                        "r_annuity": r_annuity,
                        "estate_tax_enabled": bool(D["estate_tax_enabled"]),
                        "federal_estate_exemption": float(D["federal_estate_exemption"]),
                    ...
                    _srp["post_retire_return"] = mc_post_ret
                    return (_sbals, _srp)

                _sweep_progress = st.progress(0, text="Sweeping retirement ages...")
                def _sweep_cb(step, total):
                    _sweep_progress.progress(step / total, text=f"Testing age {_sweep_start + step - 1} of {_sweep_start}-{_sweep_end}...")
                _sweep_results = engine.sweep_retirement_ages(
                    current_age=current_age_filer,
                    start_age=_sweep_start,
                    end_age=_sweep_end,
                    ...
                    build_retire_params_fn=_build_sweep_params,
                    spending_order=spending_order,
                    mc_sims=int(mc_sims),
                    mc_vol=mc_vol,
                    mc_post_ret=mc_post_ret,
                    ire_liq_equity=_ire_liq_equity,
                    progress_callback=_sweep_cb,
                )
                _sweep_progress.empty()
                st.session_state._preret_sweep = _sweep_results
        ...
        if st.session_state.projection_results:
            ...
            if st.session_state.get("_preret_mc"):
                ...
                _si_base = st.session_state.base_results
                _si_gross = _si_base.get("spendable_gross", 0) if _si_base else 0.0
                _si_inc_tax = (_si_base["total_tax"]) if _si_base else 0.0
                _si_fica = (_si_base.get("fica", 0)) if _si_base else 0.0
                _si_medicare = (_si_base.get("medicare_premiums", 0)) if _si_base else 0.0
                _si_taxes = _si_inc_tax + _si_fica + _si_medicare
                _si_savings = _employee_savings
                _si_spending = max(0, _si_gross - _inh_dist_for_cf - _si_taxes - _si_savings - mtg_payment_annual)
                _si_ret_spending = _si_spending * (D["ret_pct"] / 100)
                _si_inh_str = f" − inherited IRA dist {TEA.money(_inh_dist_for_cf)}" if _inh_dist_for_cf > 0 else ""
                st.markdown(
                    f"Gross income {TEA.money(_si_gross)} − taxes {TEA.money(_si_taxes)} "
                    f"(inc {TEA.money(_si_inc_tax)} + FICA {TEA.money(_si_fica)}"
                    + (f" + Medicare {TEA.money(_si_medicare)}" if _si_medicare > 0 else "")
                    + f"){_si_inh_str} − savings {TEA.money(_si_savings)}"
                    + (f" − mortgage {TEA.money(mtg_payment_annual)}" if mtg_payment_annual > 0 else "")
                    + f" = **{TEA.money(_si_spending)}/yr** lifestyle spending.")
                st.markdown(
                    f"Retirement spending target: **{TEA.money(_si_ret_spending)}/yr** "
                    f"({D['ret_pct']:.0f}% of current)"
                    + (f" + {TEA.money(mtg_payment_annual)}/yr mortgage" if mtg_payment_annual > 0 else "")
                    + ".")
                st.checkbox("Override: use stated spending from Spending tab",
                            key="fp_spending_override",
                            help=f"Stated: {TEA.money(D['living_expenses'])}/yr. "
                                 f"Inferred: {TEA.money(_inferred_spending)}/yr.")
                if _use_override:
                    st.caption(f"Projections use stated spending: **{TEA.money(D['living_expenses'])}/yr**")
                else:
                    st.caption(f"Projections use inferred spending: **{TEA.money(_inferred_spending)}/yr**")

                # --- Triage ---
                _mc_pct = mc["success_rate"] * 100
                if _mc_pct >= 75:
                    st.success(
                        f"You're on track to retire at {ret_age}! "
                        f"Go to **Strategy Optimizer** to maximize your after-tax estate.")
                elif _mc_pct >= 50:
                    st.warning(
                        f"You're close but not yet secure at {ret_age}. "
                        f"Go to **Savings Optimizer** to explore strategies that can close the gap.")
                else:
                    st.error(
                        f"You're not on track to retire at {ret_age}. "
                        f"Go to **Savings Optimizer** to find a path — consider saving more, "
                        f"spending less, or working longer.")

            # --- Age Sweep Display ---
            if st.session_state.get("_preret_sweep"):
                _sw = st.session_state._preret_sweep
                if _sw:
                    st.markdown("### When Can You Retire?")
                    # Find earliest age with ≥ 75% success
                    _earliest_ok = None
                    for _sr in _sw:
                        if _sr["success_rate"] >= 0.75:
                            _earliest_ok = _sr["age"]
                            break

                    if _earliest_ok is not None:
                        if _earliest_ok <= ret_age:
                            st.success(f"You could potentially retire as early as **age {_earliest_ok}** "
                                       f"(≥ 75% Monte Carlo success).")
                        else:
                            st.info(f"Earliest retirement age with ≥ 75% Monte Carlo success: **{_earliest_ok}**")
                    else:
                        st.warning("No tested retirement age achieved ≥ 75% Monte Carlo success. "
                                   "Consider increasing savings or extending your working years.")
                    ...
                    for _sr in _sw:
                        _sw_rows.append({
                            "Retire Age": _sr["age"],
                            "Years Away": _sr["years_to_ret"],
                            "MC Success": f"{_sr['success_rate'] * 100:.0f}%",
                            "Median Portfolio": f"${_sr['median_portfolio']:,.0f}",
                            "10th Pctile": f"${_sr['p10']:,.0f}",
                            "90th Pctile": f"${_sr['p90']:,.0f}",
                        })
                    _sw_df = pd.DataFrame(_sw_rows)
                    # Style: highlight earliest qualifying row
                    def _highlight_sweep(row):
                        if _earliest_ok is not None and row["Retire Age"] == _earliest_ok:
                            return ["background-color: #d4edda"] * len(row)
                        return [""] * len(row)
                    st.dataframe(_sw_df.style.apply(_highlight_sweep, axis=1),
    ...
    with tab3:
        ...
        _rr_spending_changed = (_spending_changed and st.session_state.get("retire_projection") is not None)
        if st.button("Run Retirement Readiness", type="primary", key="fp_rr_run") or _rr_spending_changed:
            ...
            _rp["expenses_at_retirement"] = _proj_spending * (rr_ret_pct / 100) * ((1 + inflation) ** years_to_ret)
        ...
        if st.session_state.retire_projection:
            ...
            with st.expander("SS Claiming Strategy Optimizer"):
                ...
                if st.button("Optimize SS Claiming Strategy", type="primary", key="fp_ss_opt_btn"):
                    ...
                    _ss_rp["expenses_at_retirement"] = _proj_spending * (rr_ret_pct / 100) * ((1 + inflation) ** years_to_ret)
    ...
    with tab4:
        ...
        # Show current allocation — plan-type-aware labels
        _plan_label_f = _plan_f if _plan_f != "None" else ""
        _plan_label_s = (D["plan_type_s"] if is_joint and salary_spouse > 0 and D["plan_type_s"] != "None" else "")
        # Use the more descriptive plan name for display
        _wp_label = _plan_label_f or _plan_label_s or "401(k)"
        st.markdown("### Current Allocation")
        _alloc_items = {}
        _pt_wp = contrib_401k_pretax_filer + contrib_401k_pretax_spouse
        _ro_wp = contrib_401k_roth_filer + contrib_401k_roth_spouse
        if _pt_wp > 0: _alloc_items[f"Pre-Tax {_wp_label}"] = TEA.money(_pt_wp)
        if _ro_wp > 0: _alloc_items[f"Roth {_wp_label}"] = TEA.money(_ro_wp)
        _trad_ira_total = contrib_trad_ira + contrib_trad_ira_spouse
        _roth_ira_total = contrib_roth_ira + contrib_roth_ira_spouse
        if _trad_ira_total > 0: _alloc_items["Trad IRA"] = TEA.money(_trad_ira_total)
        if _roth_ira_total > 0: _alloc_items["Roth IRA"] = TEA.money(_roth_ira_total)
        if contrib_hsa > 0: _alloc_items["HSA"] = TEA.money(contrib_hsa)
        if contrib_taxable > 0: _alloc_items["Taxable"] = TEA.money(contrib_taxable)
        _emp_match_total = employer_match + employer_match_spouse
        if _emp_match_total > 0: _alloc_items["Employer Match"] = TEA.money(_emp_match_total)
        if _alloc_items:
            st.dataframe(pd.DataFrame([_alloc_items]), use_container_width=True, hide_index=True)
        ...
        if st.button("Run Savings Optimizer", type="primary", key="fp_preret_run_opt"):
            ...
            # Plan-type-aware labels
            _has_ira = (employee_ira_f + employee_ira_s) > 0
            _ira_suffix = " + Trad IRA" if _has_ira else ""
            _roth_ira_suffix = " + Roth IRA" if _has_ira else ""
            ...
            # All Pre-Tax workplace plan
            _c, _d = _make_contrib(
                employee_401k_f, 0, employee_401k_s, 0,
                employee_ira_f, 0, employee_ira_s, 0,
                contrib_hsa, contrib_taxable)
            strategies.append((f"All Pre-Tax {_wp_label}{_ira_suffix}", _c, _d))

            # All Roth workplace plan
            _c, _d = _make_contrib(
                0, employee_401k_f, 0, employee_401k_s,
                0, employee_ira_f, 0, employee_ira_s,
                contrib_hsa, contrib_taxable)
            strategies.append((f"All Roth {_wp_label}{_roth_ira_suffix}", _c, _d))
            ...
            for idx, (name, c_dict, pt_ded) in enumerate(strategies):
                ...
                opt_results.append({
                    ...
                    "_accum": _acc, "_ret": _opt_ret,
                    "_contrib": c_dict, "_pt_ded": pt_ded,
                    "_bals": _opt_bals, "_rp": _opt_rp,
                })
            ...
            # --- Best Strategy: full projection + Monte Carlo ---
            _best_data = opt_results[0]
            st.markdown(f"### Best Strategy: {_best_data['Strategy']}")

            # Accumulation projection table
            _best_acc = _best_data["_accum"]
            if _best_acc and _best_acc.get("rows"):
                with st.expander("Accumulation Projection (Best Strategy)", expanded=False):
                    _ba_df = pd.DataFrame(_best_acc["rows"])
                    _ba_hide = ["Basis", "Unreal Gain"]
                    for c in list(_ba_df.columns):
                        if c in _ba_hide:
                            _ba_df = _ba_df.drop(columns=[c], errors="ignore")
                        elif _ba_df[c].dtype in ['float64', 'int64'] and (_ba_df[c] == 0).all():
                            _ba_df = _ba_df.drop(columns=[c], errors="ignore")
                    st.dataframe(_ba_df, use_container_width=True, hide_index=True)

            # Retirement projection table
            _best_ret = _best_data["_ret"]
            if _best_ret and _best_ret.get("rows"):
                with st.expander("Retirement Projection (Best Strategy)", expanded=False):
                    _br_df = pd.DataFrame(_best_ret["rows"])
                    for c in list(_br_df.columns):
                        if _br_df[c].dtype in ['float64', 'int64'] and (_br_df[c] == 0).all():
                            _br_df = _br_df.drop(columns=[c], errors="ignore")
                    st.dataframe(_br_df, use_container_width=True, hide_index=True)
                    # End-of-plan summary
                    _br_c1, _br_c2, _br_c3, _br_c4 = st.columns(4)
                    with _br_c1: st.metric("Final Portfolio", TEA.money(_best_ret.get("final_total", 0)))
                    with _br_c2: st.metric("Net Estate", TEA.money(_best_ret.get("estate", 0)))
                    with _br_c3: st.metric("Gross Estate", TEA.money(_best_ret.get("gross_estate", 0)))
                    with _br_c4: st.metric("Total Taxes", TEA.money(_best_ret.get("total_taxes", 0)))

            # Monte Carlo on best strategy
            _best_cd = _best_data["_contrib"]
            _best_pt = _best_data["_pt_ded"]
            _spouse_age_at_ret_opt = (current_age_spouse + years_to_ret) if current_age_spouse else None
            _ret_yrs_f_opt = filer_plan_through_age - ret_age
            _ret_yrs_s_opt = (spouse_plan_through_age - _spouse_age_at_ret_opt) if (_spouse_age_at_ret_opt and spouse_plan_through_age) else 0
            _ret_years_opt = max(1, max(_ret_yrs_f_opt, _ret_yrs_s_opt) + 1)
            _n_total_opt = years_to_ret + 1 + max(1, _ret_years_opt)

            def _opt_mc_run(return_seq):
                _oi = dict(income_info)
                _oi["pretax_deductions"] = _best_pt
                _oi["return_sequence"] = return_seq[:years_to_ret + 1]
                _oa = engine.run_accumulation(
                    current_age_filer, years_to_ret, copy.deepcopy(start_balances),
                    dict(_best_cd), salary_growth, pre_ret_return, _oi)
                _of = _oa["rows"][-1] if _oa["rows"] else {}
                _ob = _oa.get("final_brokerage", _of.get("Bal Taxable", 0))
                _oc = _oa.get("final_cash", 0)
                if _ire_liq_equity > 0:
                    _ob += _ire_liq_equity
                _obals = {
                    "pretax": _of.get("Bal Pre-Tax", 0),
                    "roth": _of.get("Bal Roth", 0),
                    "brokerage": _ob, "cash": _oc,
                    "taxable": _ob + _oc,
                    "brokerage_basis": _oa.get("final_basis", 0) + _ire_liq_equity,
                    "hsa": _of.get("Bal HSA", 0),
                    "conc_stock": _conc_at_retirement(years_to_ret)[0],
                    "conc_stock_basis": _conc_at_retirement(years_to_ret)[1],
                }
                _orp = _build_retire_params(_oa)
                _orp["post_retire_return"] = mc_post_ret
                _orp["heir_bracket_option"] = "same"
                _ors = return_seq[years_to_ret + 1:] if len(return_seq) > years_to_ret + 1 else None
                if _ors and len(_ors) >= _ret_years_opt:
                    _orp["return_sequence"] = _ors
                _oret = engine.run_retirement_projection(_obals, _orp, _opt_spend_order)
                return {"estate": _oret["estate"], "final_total": _oret["final_total"],
                        "retire_portfolio": _of.get("Portfolio", 0)}

            with st.spinner(f"Running Monte Carlo on best strategy ({_best_data['Strategy']})..."):
                _opt_mc = engine.run_monte_carlo(
                    _opt_mc_run, n_sims=int(mc_sims), mean_return=pre_ret_return,
                    return_std=mc_vol, n_years=_n_total_opt, seed=42,
                    mean_return_post=mc_post_ret, n_years_pre=years_to_ret)

            st.markdown("### Monte Carlo — Best Strategy")
            _mfmt_opt = lambda v: f"${float(v):,.0f}"
            _omc1, _omc2, _omc3, _omc4 = st.columns(4)
            with _omc1: st.metric("MC Success Rate", f"{_opt_mc['success_rate'] * 100:.1f}%")
            with _omc2: st.metric("Median Portfolio", _mfmt_opt(_opt_mc.get("median_portfolio", 0)))
            with _omc3: st.metric("10th Pctile", _mfmt_opt(_opt_mc.get("portfolio_p10", 0)))
        ...
        if st.button("Run Readiness Optimizer", type="primary", key="fp_run_readiness"):
            ...
            def _rr_make_run_fn(retirement_age_override=None,
                ...
                def _run_fn(return_seq):
                    ...
                    _bals = {
                        ...
                        "brokerage_basis": _acc.get("final_basis", 0) + _ire_liq_equity,
                        "hsa": _final.get("Bal HSA", 0),
                        "conc_stock": _conc_at_retirement(ytr)[0],
                        "conc_stock_basis": _conc_at_retirement(ytr)[1],
                    }
                    _rp = _build_retire_params(_acc)
                    _rp["retire_age"] = ra
            ...
            def _rr_binary_search(make_fn, lo, hi, step, target=0.75,
                ...
                mc = engine.run_monte_carlo(fn, n_sims=n_sims, mean_return=pre_ret_return,
                                            ...
                                            mean_return_post=mc_post_ret, n_years_pre=ytr)
            ...
            _rr_baseline_mc = engine.run_monte_carlo(
                ...
                mean_return_post=mc_post_ret, n_years_pre=_rr_base_ytr)
    ...
    with tab6:
        ...
        # ── Build contribution variants ──
        employee_401k_f = contrib_401k_filer
        employee_401k_s = contrib_401k_pretax_spouse + contrib_401k_roth_spouse
        employee_ira_f = contrib_trad_ira + contrib_roth_ira
        employee_ira_s = contrib_trad_ira_spouse + contrib_roth_ira_spouse

        def _u6_make_contrib(pretax_401k_f, roth_401k_f, pretax_401k_s, roth_401k_s,
                             trad_ira_f, roth_ira_f, trad_ira_s, roth_ira_s,
                             hsa, taxable):
            pf = pretax_401k_f + employer_match + trad_ira_f
            ps = pretax_401k_s + employer_match_spouse + trad_ira_s
            ...
            pt_ded = pretax_401k_f + pretax_401k_s + trad_ira_f + trad_ira_s + hsa
            ii = dict(income_info)
            ii["pretax_deductions"] = pt_ded
            return cd, ii

        # Trad-Heavy: all employee workplace plan + IRA to pre-tax
        _u6_cd_trad, _u6_ii_trad = _u6_make_contrib(
            employee_401k_f, 0, employee_401k_s, 0,
            employee_ira_f, 0, employee_ira_s, 0,
            contrib_hsa, contrib_taxable)

        # Roth-Heavy: all employee workplace plan + IRA to Roth
        _u6_cd_roth, _u6_ii_roth = _u6_make_contrib(
            0, employee_401k_f, 0, employee_401k_s,
            0, employee_ira_f, 0, employee_ira_s,
            contrib_hsa, contrib_taxable)
        ...
        _u6_roth_variants = [(lbl, strat, tgt, (ret_age + 10) if stop is None else stop)
                              for lbl, strat, tgt, stop in _u6_roth_variants]

        # ── Waterfall variants ──
        _u6_wf_variants = engine.WATERFALL_VARIANTS

        # ── Concentrated stock variants ──
        if conc_stock_value > 0:
            _u6_conc_variants = [("Hold", {"conc_stock_unwind": False})]
            _u6_conc_variants.append(("Opportunistic Unwind", {"conc_stock_unwind": True, "conc_stock_unwind_years": 0}))
            if D["conc_stock_unwind_years"] > 0:
                _u6_conc_variants.append((f"Unwind over {D['conc_stock_unwind_years']}yr",
                    {"conc_stock_unwind": True, "conc_stock_unwind_years": int(D["conc_stock_unwind_years"])}))
        else:
            _u6_conc_variants = [("Hold", {"conc_stock_unwind": False})]

        # ── Dimension summary ──
        _u6_n_combos = (len(_u6_contrib_variants) * len(_u6_wf_variants) * len(_u6_ss_variants)
                        * len(_u6_roth_variants) * len(_u6_conc_variants))
        _u6_n_accum = len(_u6_contrib_variants)
        _dim_parts = [f"{len(_u6_contrib_variants)} contrib", f"{len(_u6_wf_variants)} waterfall",
                      f"{len(_u6_ss_variants)} SS", f"{len(_u6_roth_variants)} Roth"]
        if len(_u6_conc_variants) > 1:
            _dim_parts.append(f"{len(_u6_conc_variants)} conc stock")
        st.markdown(f"**Dimensions:** {' × '.join(_dim_parts)} = **{_u6_n_combos} combos** "
                    f"({_u6_n_accum} accumulation runs)")

        if conc_stock_value > 0:
            with st.expander(f"Concentrated Stock Strategies — {conc_stock_label}"):
                st.markdown(f"""**Hold to Death** — Heirs receive stepped-up basis at death, eliminating all embedded capital gains tax.
Best when: embedded gain is very large, life expectancy is moderate, or heirs are in high tax brackets.

**Opportunistic Unwind** — Sell into available 0% capital gains bracket space each year. Slow but tax-free diversification.
Best when: other income is low enough to have 0% CG bracket room.

**Systematic Unwind by Age X** — Sell a fixed fraction each year to fully diversify by target age.
Best when: concentration risk is high, or you need diversification for risk management.

**Other strategies (not modeled):** Exchange funds, charitable remainder trusts, collars/hedging, donor-advised fund contributions of appreciated shares.""")

        if st.button("Run Unified Optimizer", type="primary", key="fp_run_unified_opt"):
            ...
            _u6_base_params = _build_retire_params(_run_accum())

            _u6_result = engine.run_unified_optimizer(
                current_age=current_age_filer,
                years_to_ret=years_to_ret,
                start_balances=copy.deepcopy(start_balances),
                contrib_variants=_u6_contrib_variants,
                waterfall_variants=_u6_wf_variants,
                ...
                conc_stock_variants=_u6_conc_variants,
            )
            ...
            _u6_ncols = 5 if conc_stock_value > 0 else 4
            _u6_wcols = st.columns(_u6_ncols)
            with _u6_wcols[0]: st.metric("Contribution Mix", _u6_best["contrib"])
            with _u6_wcols[1]: st.metric("Withdrawal Order", _u6_best["waterfall"])
            with _u6_wcols[2]: st.metric("SS Claiming", _u6_best["ss"])
            with _u6_wcols[3]: st.metric("Roth Conversion", _u6_best["roth"])
            if conc_stock_value > 0 and "conc_stock" in _u6_best:
                with _u6_wcols[4]: st.metric(conc_stock_label, _u6_best["conc_stock"])

            _u6e1, _u6e2, _u6e3, _u6e4 = st.columns(4)
            with _u6e1: st.metric("After-Tax Estate", TEA.money(_u6_best["estate"]))
            with _u6e2: st.metric("Total Taxes", TEA.money(_u6_best["taxes"]))
            with _u6e3: st.metric("Roth Converted", TEA.money(_u6_best["converted"]))
            ...
            _u6_current_combo = next(
                (c for c in _u6_result["rankings"]
                 ...
                 and c["waterfall"] == engine.WATERFALL_VARIANTS[0][0]
            ...
            if _u6_current_combo:
                ...
                if _u6_delta > 0:
                    st.success(f"**Improvement over current plan: {TEA.money(_u6_delta)}** in after-tax estate")
                elif _u6_delta == 0:
                    st.success("**Your current strategy is already optimal** across all tested combinations")
                else:
                    st.info(f"Current plan is within {TEA.money(abs(_u6_delta))} of the best combination found")
            ...
            if _u6_spreads:
                ...
                st.info(f"**Most impactful dimension: {_u6_most_impactful}** (spread: {TEA.money(_u6_spreads[_u6_most_impactful])})")

            for dim_name, options in _u6_dim.items():
                spread = _u6_spreads.get(dim_name, 0)
                with st.expander(f"{dim_name} — spread: {TEA.money(spread)}", expanded=True):
                    _dim_df = pd.DataFrame([
                        {"Option": opt, "Avg Estate": TEA.money(avg), "vs Best": TEA.money(avg - options[0][1])}
            ...
            _u6_top_df = pd.DataFrame([
                {
                    ...
                    "SS Claiming": c["ss"],
                    "Roth Conv": c["roth"],
                    "Estate": TEA.money(c["estate"]),
                    "Taxes": TEA.money(c["taxes"]),
                    "Portfolio": TEA.money(c["final_total"]),
                }
            ...
            with st.expander("Retirement Phase"):
                ...
                if _u6_ret.get("rows"):
                    ...
                    with _u6r1: st.metric("Final Portfolio", TEA.money(_u6_ret["final_total"]))
                    with _u6r2: st.metric("Final Estate", TEA.money(_u6_ret["estate"]))
                    with _u6r3: st.metric("Total Taxes", TEA.money(_u6_ret["total_taxes"]))
                    with _u6r4: st.metric("Roth Converted", TEA.money(_u6_ret.get("total_converted", 0)))

            # ── Monte Carlo on Winning Strategy ──
            st.divider()
            _u6_best_rank = _u6_result["rankings"][0]
            # Find matching contribution variant
            _u6_mc_cd = dict(contrib_dict)
            _u6_mc_ii = dict(income_info)
            for _cv_label, _cv_dict, _cv_ii in _u6_contrib_variants:
                if _cv_label == _u6_best_rank["contrib"]:
                    _u6_mc_cd = _cv_dict
                    _u6_mc_ii = _cv_ii
                    break

            _u6_mc_wf = _u6_best_rank["_w_order"]
            _u6_mc_sfc = _u6_best_rank["_s_fc"]
            _u6_mc_ssc = _u6_best_rank.get("_s_sc")
            _u6_mc_rs = _u6_best_rank["_r_strategy"]
            _u6_mc_rt = _u6_best_rank["_r_target"]
            _u6_mc_rstop = _u6_best_rank.get("_r_stop")
            _u6_spouse_age_at_ret = (current_age_spouse + years_to_ret) if current_age_spouse else None
            _u6_ret_yrs_f = filer_plan_through_age - ret_age
            _u6_ret_yrs_s = (spouse_plan_through_age - _u6_spouse_age_at_ret) if (_u6_spouse_age_at_ret and spouse_plan_through_age) else 0
            _u6_ret_years = max(1, max(_u6_ret_yrs_f, _u6_ret_yrs_s) + 1)
            _u6_n_total = years_to_ret + 1 + max(1, _u6_ret_years)

            def _u6_mc_run(return_seq):
                _oi = dict(_u6_mc_ii)
                _oi["return_sequence"] = return_seq[:years_to_ret + 1]
                _oa = engine.run_accumulation(
                    current_age_filer, years_to_ret, copy.deepcopy(start_balances),
                    dict(_u6_mc_cd), salary_growth, pre_ret_return, _oi)
                _of = _oa["rows"][-1] if _oa["rows"] else {}
                _ob = _oa.get("final_brokerage", _of.get("Bal Taxable", 0))
                _oc = _oa.get("final_cash", 0)
                if _ire_liq_equity > 0:
                ...
                _orp = _build_retire_params(_oa)
                _orp["post_retire_return"] = mc_post_ret
                _orp["ss_filer_claim_age"] = _u6_mc_sfc
                if _u6_mc_ssc is not None:
                    _orp["ss_spouse_claim_age"] = _u6_mc_ssc
                if _u6_mc_rs != "none":
                    _orp["roth_conversion_strategy"] = _u6_mc_rs
                    _orp["roth_conversion_target_agi"] = _u6_mc_rt
                    _orp["roth_conversion_stop_age"] = _u6_mc_rstop if _u6_mc_rstop is not None else (ret_age + 10)
                _ors = return_seq[years_to_ret + 1:] if len(return_seq) > years_to_ret + 1 else None
                if _ors and len(_ors) >= _u6_ret_years:
                    _orp["return_sequence"] = _ors
                _oret = engine.run_retirement_projection(_obals, _orp, list(_u6_mc_wf))
                return {"estate": _oret["estate"], "final_total": _oret["final_total"],
                        "retire_portfolio": _of.get("Portfolio", 0)}

            with st.spinner("Running Monte Carlo on winning strategy..."):
                _u6_mc = engine.run_monte_carlo(
                    _u6_mc_run, n_sims=int(mc_sims), mean_return=pre_ret_return,
                    return_std=mc_vol, n_years=_u6_n_total, seed=42,
                    mean_return_post=mc_post_ret, n_years_pre=years_to_ret)

            st.markdown("### Monte Carlo — Winning Strategy")
            _u6_mfmt = lambda v: f"${float(v):,.0f}"
            _u6m1, _u6m2, _u6m3, _u6m4 = st.columns(4)
            with _u6m1: st.metric("MC Success Rate", f"{_u6_mc['success_rate'] * 100:.1f}%")
            with _u6m2: st.metric("Median Portfolio", _u6_mfmt(_u6_mc.get("median_portfolio", 0)))
            with _u6m3: st.metric("10th Pctile", _u6_mfmt(_u6_mc.get("portfolio_p10", 0)))
            with _u6m4: st.metric("90th Pctile", _u6_mfmt(_u6_mc.get("portfolio_p90", 0)))
            st.caption("Monte Carlo portfolio at end of plan (excludes home equity)")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    with st.expander("Concentrated Stock Position"):
        st.caption("Large single-stock position with low cost basis (e.g., legacy company stock). "
                   "Tracked separately from diversified brokerage for withdrawal and unwind planning.")
        _cs_c1, _cs_c2 = st.columns(2)
        with _cs_c1:
            w_num("Market Value", "conc_stock_value", step=10000.0)
            w_num("Cost Basis", "conc_stock_basis", step=10000.0)
            if D["conc_stock_value"] > 0:
                _cs_gp = max(0.0, 1.0 - D["conc_stock_basis"] / D["conc_stock_value"]) if D["conc_stock_value"] > 0 else 0.0
                st.caption(f"Embedded gain: {_cs_gp:.1%}")
        with _cs_c2:
            st.text_input("Label (e.g. AAPL)", value=D["conc_stock_label"],
                          key="_w_conc_stock_label", on_change=_sync, args=("conc_stock_label",))
            w_num("Dividend Yield %", "conc_stock_div_yield", step=0.1, format="%.2f")
        if D["conc_stock_value"] > 0:
            w_check("Unwind concentrated position", "conc_stock_unwind")
            if D["conc_stock_unwind"]:
                w_num("Unwind over X years (0 = opportunistic)", "conc_stock_unwind_years",
                      min_value=0, step=1,
                      help="Number of years to spread the unwind. 1 = sell all immediately. 0 = sell opportunistically into available 0% CG bracket space.")
    ...
    with st.expander("Inherited IRA(s)"):
        ...
        if D["inh_ira_f"] > 0:
            ...
            # --- RMD + Recommendation ---
            _g_age_f = TEA.age_at_date(D["filer_dob"], dt.date.today())
            _g_rule_f = "10_year" if D["inh_rule_f"] == "10-Year Rule" else "lifetime"
            _g_rmd_f = bool(D["inh_rmd_req_f"])
            if _g_rmd_f or _g_rule_f == "lifetime":
                _g_div_f = engine.SINGLE_LIFE_TABLE.get(_g_age_f, 1.0)
                _g_min_rmd_f = D["inh_ira_f"] / _g_div_f if _g_div_f > 0 else D["inh_ira_f"]
                st.info(f"Minimum annual RMD (year 1): {TEA.money(_g_min_rmd_f)}")
            _g_yrs_f = int(D["inh_yrs_f"]) if _g_rule_f == "10_year" else 0
            _g_income = D["salary_filer"] + D["salary_spouse"] + D["other_income"]
            _g_state = D["state_tax_rate"] / 100
            _g_growth = D["pre_ret_return"] / 100
            _g_brgr = D["bracket_growth"] / 100
            _g_horizon_f = max(5, int(D["filer_plan_age"]) - _g_age_f) if _g_rule_f == "lifetime" else None
            _g_rec_f = engine.calc_inherited_ira_recommendation(
                D["inh_ira_f"], _g_yrs_f, _g_age_f, _g_rmd_f,
                _g_income, filing_status, state_rate=_g_state,
                growth_rate=_g_growth, inflation=_g_brgr,
                rule=_g_rule_f, horizon_years=_g_horizon_f)
            if _g_rec_f:
                st.markdown("---")
                st.markdown(f"**Distribution Recommendation:** Fill to top of {_g_rec_f['target_bracket_rate']:.0%} bracket")
                rc1, rc2 = st.columns(2)
                if _g_rule_f == "10_year":
                    with rc1:
                        st.metric("Recommended Annual", TEA.money(_g_rec_f["recommended_annual"]))
                        st.caption(f"Stays in {_g_rec_f['target_bracket_rate']:.0%} bracket (current rate: {_g_rec_f['marginal_without']:.0%})")
                    with rc2:
                        st.metric("Est. Tax Savings vs Final-Year Lump", TEA.money(_g_rec_f["total_tax_lump"] - _g_rec_f["total_tax_recommended"]))
                    if _g_rec_f.get("final_yr_lump") and _g_rec_f["final_yr_lump"] > _g_rec_f["recommended_annual"] * 1.5:
                        st.error(f"Without spreading: final year forces {TEA.money(_g_rec_f['final_yr_lump'])} lump sum → {_g_rec_f['final_yr_rate']:.0%} bracket")
                else:
                    with rc1:
                        if _g_rec_f.get("recommended_additional", 0) > 0:
                            st.metric("Recommended Annual Total", TEA.money(_g_rec_f["recommended_annual"]))
                            st.caption(f"Min RMD + {TEA.money(_g_rec_f['recommended_additional'])} additional")
                        else:
                            st.metric("Year-1 Minimum RMD", TEA.money(_g_rec_f["recommended_annual"]))
                            st.caption("Min RMD already fills or exceeds current bracket")
                    with rc2:
                        if _g_rec_f.get("tax_savings", 0) > 0:
                            st.metric("Est. Tax Savings vs Min-Only", TEA.money(_g_rec_f["tax_savings"]))
                        if _g_rec_f.get("peak_min_only_dist", 0) > 0:
                            st.metric("Peak Future RMD (min-only)", TEA.money(_g_rec_f["peak_min_only_dist"]))
                            st.caption(f"At age {_g_rec_f['peak_min_only_age']} → {_g_rec_f['peak_min_only_rate']:.0%} bracket")
                if _g_rec_f.get("spill_desc"):
                    st.info(_g_rec_f["spill_desc"])
                if _g_rec_f.get("warning"):
                    st.warning(_g_rec_f["warning"])
                with st.expander("Year-by-Year Schedule"):
                    _g_sched_f = pd.DataFrame(_g_rec_f["schedule"])
                    _g_sched_f["marginal_rate"] = _g_sched_f["marginal_rate"].apply(lambda x: f"{x:.0%}")
                    st.dataframe(_g_sched_f, use_container_width=True, hide_index=True)
        if is_joint:
            ...
            if D["inh_ira_s"] > 0:
                ...
                w_num("Additional Annual Distribution — Spouse", "inh_add_s", step=1000.0,
                      help="Extra amount to distribute each year beyond the minimum.")
                # --- Spouse RMD + Recommendation ---
                _g_sp_dob = D.get("spouse_dob") or D["filer_dob"]
                _g_sp_age = TEA.age_at_date(_g_sp_dob, dt.date.today())
                _g_rule_s = "10_year" if D["inh_rule_s"] == "10-Year Rule" else "lifetime"
                _g_rmd_s = bool(D["inh_rmd_req_s"])
                if _g_rmd_s or _g_rule_s == "lifetime":
                    _g_div_s = engine.SINGLE_LIFE_TABLE.get(_g_sp_age, 1.0)
                    _g_min_rmd_s = D["inh_ira_s"] / _g_div_s if _g_div_s > 0 else D["inh_ira_s"]
                    st.info(f"Spouse minimum annual RMD (year 1): {TEA.money(_g_min_rmd_s)}")
                _g_yrs_s = int(D["inh_yrs_s"]) if _g_rule_s == "10_year" else 0
                _gs_income = D["salary_filer"] + D["salary_spouse"] + D["other_income"]
                _gs_state = D["state_tax_rate"] / 100
                _gs_growth = D["pre_ret_return"] / 100
                _gs_brgr = D["bracket_growth"] / 100
                _gs_horizon = max(5, int(D["spouse_plan_age"]) - _g_sp_age) if _g_rule_s == "lifetime" else None
                _g_rec_s = engine.calc_inherited_ira_recommendation(
                    D["inh_ira_s"], _g_yrs_s, _g_sp_age, _g_rmd_s,
                    _gs_income, filing_status, state_rate=_gs_state,
                    growth_rate=_gs_growth, inflation=_gs_brgr,
                    rule=_g_rule_s, horizon_years=_gs_horizon)
                if _g_rec_s:
                    st.markdown("---")
                    st.markdown(f"**Spouse Distribution Recommendation:** Fill to top of {_g_rec_s['target_bracket_rate']:.0%} bracket")
                    sc1, sc2 = st.columns(2)
                    if _g_rule_s == "10_year":
                        with sc1:
                            st.metric("Recommended Annual", TEA.money(_g_rec_s["recommended_annual"]))
                            st.caption(f"Stays in {_g_rec_s['target_bracket_rate']:.0%} bracket")
                        with sc2:
                            st.metric("Est. Tax Savings vs Final-Year Lump", TEA.money(_g_rec_s["total_tax_lump"] - _g_rec_s["total_tax_recommended"]))
                    else:
                        with sc1:
                            if _g_rec_s.get("recommended_additional", 0) > 0:
                                st.metric("Recommended Annual Total", TEA.money(_g_rec_s["recommended_annual"]))
                                st.caption(f"Min RMD + {TEA.money(_g_rec_s['recommended_additional'])} additional")
                            else:
                                st.metric("Year-1 Minimum RMD", TEA.money(_g_rec_s["recommended_annual"]))
                        with sc2:
                            if _g_rec_s.get("tax_savings", 0) > 0:
                                st.metric("Est. Tax Savings vs Min-Only", TEA.money(_g_rec_s["tax_savings"]))
                            if _g_rec_s.get("peak_min_only_dist", 0) > 0:
                                st.metric("Peak Future RMD (min-only)", TEA.money(_g_rec_s["peak_min_only_dist"]))
                    if _g_rec_s.get("warning"):
                        st.warning(_g_rec_s["warning"])
                    with st.expander("Spouse Year-by-Year Schedule"):
                        _gs_sched = pd.DataFrame(_g_rec_s["schedule"])
                        _gs_sched["marginal_rate"] = _gs_sched["marginal_rate"].apply(lambda x: f"{x:.0%}")
                        st.dataframe(_gs_sched, use_container_width=True, hide_index=True)
    ...
    with col2:
        ...
        w_check("Death benefit grows with cash value", "life_db_grows")
        w_check("Spend cash value in retirement", "life_spend_cv")

    # --- Additional Accounts ---
    st.divider()
    st.subheader("Additional Accounts")
    st.caption("Add accounts beyond what's listed above (e.g., a second 401k, spouse's brokerage).")

    with st.popover("+ Add Account", use_container_width=False):
        st.caption("Select account type:")
        for _atype in _ADDL_ACCOUNT_TYPES:
            # Singleton check: type already exists (static non-zero OR already added)
            _singleton_exists = False
            if _atype in _SINGLETON_ACCT_TYPES:
                _already_added = any(a["type"] == _atype for a in st.session_state.accounts)
                if _atype == "Life Insurance":
                    _singleton_exists = D["life_cash_value"] > 0 or D["life_death_benefit"] > 0 or _already_added
                elif _atype == "Concentrated Stock":
                    _singleton_exists = D["conc_stock_value"] > 0 or _already_added
                elif _atype == "Cash":
                    _singleton_exists = _already_added  # allow adding if static is zero or not
                elif _atype == "HSA":
                    _singleton_exists = _already_added
            # Inherited IRA: max 1 per owner
            if _atype == "Inherited IRA":
                _inh_added = [a for a in st.session_state.accounts if a["type"] == "Inherited IRA"]
                if len(_inh_added) >= 2:
                    _singleton_exists = True
            if st.button(_atype, key=f"_acct_add_{_atype}", use_container_width=True,
                         disabled=_singleton_exists):
                st.session_state.accounts.append(
                    _make_default_account(_atype, st.session_state._acct_next_id))
                st.session_state._acct_next_id += 1
                for _k in [k for k in st.session_state if k.startswith("_acct_")
                           and k != "_acct_next_id"]:
                    del st.session_state[_k]
                st.rerun()

    # Render each additional account
    _acct_to_remove = None
    for idx, acct in enumerate(st.session_state.accounts):
        t = acct["type"]
        _display_label = acct.get("label") or f"{t} — {acct['owner']}"
        st.markdown(f"**{_display_label}**")

        c1, c2, c3, c4, c5 = st.columns([2.5, 1.5, 2, 2, 0.5])
        with c1:
            st.caption(f"Type: {t}")
        with c2:
            _owner_opts = ["Filer", "Spouse"] if is_joint else ["Filer"]
            if t in {"Cash", "HSA", "Concentrated Stock", "Roth 401(k)"}:
                st.caption(f"Owner: {acct['owner']}")
            else:
                acct["owner"] = st.selectbox("Owner", _owner_opts,
                    index=_owner_opts.index(acct["owner"]) if acct["owner"] in _owner_opts else 0,
                    key=f"_acct_own_{idx}", label_visibility="collapsed")
        with c3:
            acct["balance"] = st.number_input("Balance", value=float(acct["balance"]),
                min_value=0.0, step=10000.0, key=f"_acct_bal_{idx}")
        with c4:
            acct["label"] = st.text_input("Label", value=acct.get("label", ""),
                key=f"_acct_lbl_{idx}", placeholder=t)
        with c5:
            if st.button("X", key=f"_acct_rm_{idx}"):
                _acct_to_remove = idx

        # Type-specific extra fields
        if t == "Taxable Brokerage":
            acct["basis"] = st.number_input("Cost Basis", value=float(acct.get("basis", 0)),
                min_value=0.0, step=10000.0, key=f"_acct_basis_{idx}")

        elif t == "Annuity":
            _ac1, _ac2 = st.columns(2)
            with _ac1:
                acct["basis"] = st.number_input("Basis", value=float(acct.get("basis", 0)),
                    min_value=0.0, step=5000.0, key=f"_acct_basis_{idx}")
            with _ac2:
                acct["monthly_contribution"] = st.number_input("Monthly Contribution",
                    value=float(acct.get("monthly_contribution", 0)),
                    min_value=0.0, step=50.0, key=f"_acct_mc_{idx}")

        elif t == "Life Insurance":
            _lc1, _lc2 = st.columns(2)
            with _lc1:
                acct["basis"] = st.number_input("Cost Basis", value=float(acct.get("basis", 0)),
                    step=1000.0, key=f"_acct_basis_{idx}")
                acct["monthly_premium"] = st.number_input("Monthly Premium",
                    value=float(acct.get("monthly_premium", 0)), step=50.0, key=f"_acct_mp_{idx}")
                acct["pay_to_age"] = st.number_input("Pay to Age",
                    value=float(acct.get("pay_to_age", 100)), step=1.0, key=f"_acct_pta_{idx}")
            with _lc2:
                acct["death_benefit"] = st.number_input("Death Benefit",
                    value=float(acct.get("death_benefit", 0)), step=10000.0, key=f"_acct_db_{idx}")
                _po_opts = ["Filer Death", "Spouse Death", "Last to Die"]
                acct["payout"] = st.selectbox("Pays on", _po_opts,
                    index=_po_opts.index(acct.get("payout", "Last to Die")), key=f"_acct_po_{idx}")
                acct["db_grows"] = st.checkbox("DB grows with CV", value=acct.get("db_grows", False),
                    key=f"_acct_dbg_{idx}")
                acct["spend_cv"] = st.checkbox("Spend CV in retirement", value=acct.get("spend_cv", False),
                    key=f"_acct_scv_{idx}")

        elif t == "Concentrated Stock":
            _cc1, _cc2 = st.columns(2)
            with _cc1:
                acct["basis"] = st.number_input("Cost Basis", value=float(acct.get("basis", 0)),
                    step=10000.0, key=f"_acct_basis_{idx}")
            with _cc2:
                acct["div_yield"] = st.number_input("Div Yield %",
                    value=float(acct.get("div_yield", 0)), step=0.1, key=f"_acct_dy_{idx}")
            if acct["balance"] > 0:
                acct["unwind"] = st.checkbox("Unwind position", value=acct.get("unwind", False),
                    key=f"_acct_uw_{idx}")
                if acct["unwind"]:
                    acct["unwind_years"] = st.number_input("Unwind over X years (0=opportunistic)",
                        value=int(acct.get("unwind_years", 0)), min_value=0, step=1,
                        key=f"_acct_uwy_{idx}")

        elif t == "Inherited IRA":
            acct["rule"] = st.selectbox("Distribution Rule",
                ["10-Year Rule", "Lifetime RMDs"],
                index=0 if acct.get("rule") == "10-Year Rule" else 1,
                key=f"_acct_rule_{idx}")
            if acct["rule"] == "10-Year Rule":
                acct["years_remaining"] = st.number_input("Years Remaining",
                    value=int(acct.get("years_remaining", 10)), min_value=1, max_value=10,
                    step=1, key=f"_acct_yrs_{idx}")
                acct["rmd_required"] = st.checkbox("Original owner was taking RMDs?",
                    value=acct.get("rmd_required", False), key=f"_acct_rmd_{idx}")
            acct["additional_distribution"] = st.number_input("Additional Annual Distribution",
                value=float(acct.get("additional_distribution", 0)), step=1000.0,
                key=f"_acct_addl_{idx}")

        st.markdown("---")

    if _acct_to_remove is not None:
        st.session_state.accounts.pop(_acct_to_remove)
        for _k in [k for k in st.session_state if k.startswith("_acct_") and k != "_acct_next_id"]:
            del st.session_state[_k]
        st.rerun()
...
# ══════════════════════════════════════════════════════════════════════
    ...
    with col2:
        w_num("Other Income", "other_income", step=500.0)
        if D["other_income"] > 0:
            w_check("Tax-free (e.g., disability, Roth, gifts)", "other_income_tax_free")
            w_check("Inflate with inflation", "other_income_inflate")
            _oc1, _oc2 = st.columns(2)
            with _oc1:
                if D["other_income_start_age"] == 0:
                    D["other_income_start_age"] = _oi_default_age
                w_num("Start age", "other_income_start_age", min_value=18, max_value=110, step=1, format="%d")
            with _oc2:
    ...
    _fi_birth_year = (current_year - _filer_age_recv) if _filer_age_recv else (current_year - 70)
    if st.button("Add Income", key="_btn_add_income"):
        st.session_state.future_income.append({
            "name": "", "amount": 0.0,
            "start_age": _filer_age_recv, "end_age": 0,
            "inflates": False, "tax_type": "Ordinary Income", "basis": 0.0,
        })
        st.rerun()
    _fi_to_remove = None
    for idx, fi in enumerate(st.session_state.future_income):
        ...
        _fi_start_yr = _fi_birth_year + fi["start_age"]
        _fi_end_yr = _fi_birth_year + fi["end_age"] if fi["end_age"] > 0 else 0
        with c3:
            _new_start_yr = st.number_input("Start Year", value=int(_fi_start_yr), min_value=2020, max_value=current_year + 50, step=1, key=f"_fi_start_{idx}")
            fi["start_age"] = _new_start_yr - _fi_birth_year
        with c4:
            _new_end_yr = st.number_input("End Year (0=ongoing)", value=int(_fi_end_yr), min_value=0, max_value=current_year + 50, step=1, key=f"_fi_end_{idx}")
            fi["end_age"] = (_new_end_yr - _fi_birth_year) if _new_end_yr > 0 else 0
        # Backward compat: migrate old taxable boolean to tax_type
        if "taxable" in fi and "tax_type" not in fi:
            fi["tax_type"] = "Ordinary Income" if fi["taxable"] else "Non-Taxable"
            fi.setdefault("basis", 0.0)
        fi.setdefault("tax_type", "Ordinary Income")
        fi.setdefault("basis", 0.0)
        _fi_tax_types = ["Non-Taxable", "Ordinary Income", "Capital Gains"]
        c5, c6, c7 = st.columns([2, 2, 1])
        with c5: fi["inflates"] = st.checkbox("Inflates?", value=fi["inflates"], key=f"_fi_inf_{idx}")
        with c6: fi["tax_type"] = st.selectbox("Tax Type", _fi_tax_types, index=_fi_tax_types.index(fi["tax_type"]), key=f"_fi_tax_{idx}")
        with c7:
            if st.button("Remove", key=f"_fi_rm_{idx}"):
                _fi_to_remove = idx
        if fi["tax_type"] == "Capital Gains":
            fi["basis"] = st.number_input("Cost Basis $", value=float(fi["basis"]), min_value=0.0, step=1000.0, key=f"_fi_basis_{idx}")
        st.markdown("---")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    _ae_sources = ["Achieving (Waterfall)", "Taxable – Cash", "Taxable – Brokerage", "Pre-Tax – IRA/401k", "Annuity", "Roth", "Life Insurance (loan)"]
    if is_joint:
    ...
    _ae_birth_year = (current_year - _filer_age_now) if _filer_age_now else (current_year - 70)

    _ae_categories = ["Lifestyle", "Medical", "Travel", "Gifts to Family", "Gifts to Charity", "Alimony"]
    with st.popover("➕ Add Expense", use_container_width=False):
        st.caption("Select a category:")
        for _cat in _ae_categories:
            if st.button(_cat, key=f"_ae_cat_{_cat}", use_container_width=True):
                st.session_state.additional_expenses.append({
                    "name": _cat, "net_amount": 0.0,
                    "start_age": _filer_age_now, "end_age": 0,
                    "inflates": False, "source": "Available Cash",
                    "gross_needed": 0.0, "calculated": False, "tax_impact": None,
                })
                for _k in [k for k in st.session_state if k.startswith("_ae_") and k != "_ae_next_id"]:
                    del st.session_state[_k]
                st.rerun()
    ...
    for idx, ae in enumerate(st.session_state.additional_expenses):
        ...
        _ae_start_yr = _ae_birth_year + ae["start_age"]
        _ae_end_yr = _ae_birth_year + ae["end_age"] if ae["end_age"] > 0 else 0
        with c3:
            _new_ae_start_yr = st.number_input("Start Year", value=int(_ae_start_yr), min_value=2020, max_value=current_year + 50, step=1, key=f"_ae_start_{idx}")
            ae["start_age"] = _new_ae_start_yr - _ae_birth_year
        with c4:
            _new_ae_end_yr = st.number_input("End Year (0=one-time)", value=int(_ae_end_yr), min_value=0, max_value=current_year + 50, step=1, key=f"_ae_end_{idx}")
            ae["end_age"] = (_new_ae_end_yr - _ae_birth_year) if _new_ae_end_yr > 0 else 0
        c5, c6, c7 = st.columns([2, 3, 1])
        with c5: ae["inflates"] = st.checkbox("Inflates?", value=ae["inflates"], key=f"_ae_inf_{idx}")
        with c6:
            _ae_display_src = ae["source"] if ae["source"] != "Available Cash" else "Achieving (Waterfall)"
            _src_sel = st.selectbox("Source", _ae_sources, index=(_ae_sources.index(_ae_display_src) if _ae_display_src in _ae_sources else 0), key=f"_ae_src_{idx}")
            ae["source"] = "Available Cash" if _src_sel == "Achieving (Waterfall)" else _src_sel
        with c7:
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.divider()
    st.subheader("Lifetime Appreciated Stock Donations")
    st.caption("Donate appreciated stock from your brokerage to charity. "
               "Avoids capital gains tax on the appreciation and provides a "
               "charitable deduction at full market value. Useful before age 70½ "
               "(when QCDs aren't available) or for giving beyond QCD limits.")
    _sd_c1, _sd_c2 = st.columns([2, 1])
    with _sd_c1:
        w_num("Annual stock donation amount", "stock_donation_annual", step=1000.0,
              help="Amount of appreciated stock to donate annually from your brokerage account.")
    with _sd_c2:
        w_num("Stop at age", "stock_donation_end_age", step=1,
              help="Stop stock donations at this age (e.g., 70 if switching to QCDs at 70½).")
    D["stock_donation_additional"] = st.checkbox(
        "In addition to annual charitable contributions above",
        value=D["stock_donation_additional"], key="_w_stock_donation_additional",
        help="If checked, stock donations are extra giving on top of cash contributions. "
             "If unchecked, stock donations replace part of the cash giving (reduces cash spending need).")

    st.divider()
    st.subheader("Charitable Estate Bequest")
    st.caption("At death, allocate a charitable bequest from the most tax-inefficient "
               "accounts first: Pre-Tax IRA (heirs would pay full income tax), then "
               "annuity gains. Roth and life insurance always stay with heirs.")
    _bequest_mode = st.radio("Bequest type", ["Percentage of estate", "Fixed dollar amount"],
                             index=0 if D["charitable_bequest_mode"] == "pct" else 1,
                             key="_w_bequest_mode", horizontal=True)
    D["charitable_bequest_mode"] = "pct" if _bequest_mode == "Percentage of estate" else "fixed"
    if D["charitable_bequest_mode"] == "pct":
        w_num("Charitable bequest (%)", "charitable_bequest_pct", step=1.0, format="%.0f",
              help="Percentage of gross estate directed to charity at death.")
    else:
        w_num("Charitable bequest ($)", "charitable_bequest_fixed", step=10000.0,
              help="Fixed dollar amount directed to charity at death.")
        D["charitable_bequest_inflate"] = st.checkbox(
            "Inflate with CPI",
            value=D["charitable_bequest_inflate"], key="_w_bequest_inflate",
            help="If checked, the bequest amount grows with inflation each year.")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    # Pensions — full amounts for projection engine (engine handles start age internally)
    pension_filer = D["pension_filer"]
    pension_spouse = D["pension_spouse"] if is_joint else 0.0

    # Save D's clean values before aggregation (restored at top of next rerun)
    st.session_state["_d_pre_aggregate"] = {k: D[k] for k in _ACCT_AGGREGATED_KEYS if k in D}
    # Aggregate additional accounts into D keys before reading balances
    _aggregate_additional_accounts()
    ...
    # Concentrated stock position
    conc_stock_value = D["conc_stock_value"]
    conc_stock_basis = min(D["conc_stock_basis"], conc_stock_value) if conc_stock_value > 0 else 0.0
    conc_stock_gain_pct = max(0.0, 1.0 - conc_stock_basis / conc_stock_value) if conc_stock_value > 0 else 0.0
    conc_stock_label = D["conc_stock_label"] or "Concentrated Stock"
    conc_stock_div_yield = D["conc_stock_div_yield"] / 100
    ...
    _oi_tax_free = bool(D["other_income_tax_free"])
    # Gate other income by start/end age
    _oi_start = int(D["other_income_start_age"]) or 0
    _oi_end = int(D["other_income_end_age"]) or 0
    _oi_in_range = ((_oi_start == 0 or current_age_filer >= _oi_start)
                    and (_oi_end == 0 or current_age_filer < _oi_end))
    _oi_amount = D["other_income"] if _oi_in_range else 0.0
    other_income = 0.0 if _oi_tax_free else _oi_amount
    _oi_cashflow_taxfree = _oi_amount if _oi_tax_free else 0.0
    ...
    if D["use_asset_alloc"]:
        ...
        r_life = _compute_aa_return("life")
    else:
        r_cash = D["r_cash"] / 100
    ...
    # Age-gate pensions for current-year tax estimator (engine handles start age in projections)
    _pen_f_now = pension_filer if current_age_filer >= int(D["pension_filer_age"]) else 0.0
    _pen_s_now = (pension_spouse if current_age_spouse and current_age_spouse >= int(D["pension_spouse_age"]) else 0.0) if is_joint else 0.0
    taxable_pensions_total = _pen_f_now + _pen_s_now
    ...
    # --- Active future income → base tax inputs ---
    _fi_taxfree = 0.0
    for fi in st.session_state.get("future_income", []):
        if fi.get("amount", 0) <= 0:
            continue
        _fi_sa = fi.get("start_age", 999)
        _fi_ea = fi.get("end_age", 0)
        if _fi_sa > current_age_filer or (_fi_ea > 0 and _fi_ea <= current_age_filer):
            continue
        _fi_tt = fi.get("tax_type", "Ordinary Income")
        if _fi_tt == "Non-Taxable":
            _fi_taxfree += fi["amount"]
        elif _fi_tt == "Ordinary Income":
            other_income += fi["amount"]
        elif _fi_tt == "Capital Gains":
            cap_gain_loss += fi["amount"]

    # --- Inherited IRA current-year distributions → base tax ---
    _inh_dist_total = 0.0
    if D["inh_ira_f"] > 0:
        _idf_rule = "10_year" if D["inh_rule_f"] == "10-Year Rule" else "lifetime"
        _idf_rmd = bool(D["inh_rmd_req_f"])
        if _idf_rmd or _idf_rule == "lifetime":
            _idf_div = engine.SINGLE_LIFE_TABLE.get(current_age_filer, 1.0)
            _inh_dist_total += D["inh_ira_f"] / _idf_div if _idf_div > 0 else D["inh_ira_f"]
        _inh_dist_total += float(D["inh_add_f"])
    if is_joint and D["inh_ira_s"] > 0:
        _ids_rule = "10_year" if D["inh_rule_s"] == "10-Year Rule" else "lifetime"
        _ids_rmd = bool(D["inh_rmd_req_s"])
        if _ids_rmd or _ids_rule == "lifetime":
            _ids_age = current_age_spouse or current_age_filer
            _ids_div = engine.SINGLE_LIFE_TABLE.get(_ids_age, 1.0)
            _inh_dist_total += D["inh_ira_s"] / _ids_div if _ids_div > 0 else D["inh_ira_s"]
        _inh_dist_total += float(D["inh_add_s"])
    ...
    def current_inputs():
        return {
            ...
            "taxable_ira": float(baseline_pretax_distributions + _inh_dist_total), "rmd_amount": float(rmd_total),
            ...
            "ordinary_tax_only": 0.0, "cashflow_taxfree": float(_fi_taxfree + _oi_cashflow_taxfree),
    ...
    def current_assets():
        ...
        return {
            "taxable": {"cash": max(0.0, float(taxable_cash_bal) - float(emergency_fund)),
                        ...
                        "concentrated": float(conc_stock_value),
                        "concentrated_basis": float(conc_stock_basis)},
    ...
    def _build_tab3_params():
        ...
        return {
            ...
            "pension_filer": float(_pen_f_now), "pension_spouse": float(_pen_s_now),
            ...
            "wages": float(wages), "tax_exempt_interest": float(tax_exempt_interest),
            "other_income": float(other_income), "other_income_tax_free": bool(D["other_income_tax_free"]),
            ...
            "state_estate_exemption": float(D["state_estate_exemption"]),
            # Charitable bequest
            "charitable_bequest_pct": float(D["charitable_bequest_pct"]) / 100.0,
            "charitable_bequest_fixed": float(D["charitable_bequest_fixed"]),
            "charitable_bequest_inflate": bool(D["charitable_bequest_inflate"]),
            "charitable_bequest_mode": D["charitable_bequest_mode"],
            "stock_donation_annual": float(D["stock_donation_annual"]),
            "stock_donation_end_age": int(D["stock_donation_end_age"]),
            "stock_donation_additional": bool(D["stock_donation_additional"]),
    ...
    if _is_working:
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            ...
            "Retirement Projection", "Strategy Optimizer"])
    else:
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    # ================================================================
    # PRE-RETIREMENT TABS (tabs 2-5) — only when working
    # Must run before Tab 1 so _fp_employee_savings is in session state
    # ================================================================
    if _is_working:
        _fp_pre_ret_tabs(tab2, tab4, tab3, tab5, D, is_joint, filing_status,
                         filer_dob, spouse_dob, current_year, inflation, bracket_growth,
                         medicare_growth, pension_cola, spending_order, surplus_destination,
                         current_age_filer, current_age_spouse,
                         filer_plan_through_age, spouse_plan_through_age,
    ...
    with tab1:
        ...
        _tab1_savings = st.session_state.get("_fp_employee_savings", 0.0) if _is_working else 0.0
        TEA.display_tax_return(r, mortgage_pmt=float(mortgage_payment),
                               filer_65=filer_65_plus, spouse_65=spouse_65_plus,
                               savings=_tab1_savings)

        # ── Net Liquidation Value ──────────────────────────────
        _liq_rows = []  # (label, gross, taxable_amt)

        _cash_gross = float(taxable_cash_bal) + float(emergency_fund)
        if _cash_gross > 0:
            _liq_rows.append(("Cash & Emergency Fund", _cash_gross, 0.0))

        _brk_gross = float(taxable_brokerage_bal)
        _brk_taxable = _brk_gross * float(brokerage_gain_pct)
        if _brk_gross > 0:
            _liq_rows.append(("Brokerage", _brk_gross, _brk_taxable))

        _conc_gross = float(conc_stock_value)
        _conc_taxable = _conc_gross * float(conc_stock_gain_pct)
        if _conc_gross > 0:
            _liq_rows.append((f"{conc_stock_label}", _conc_gross, _conc_taxable))

        _pt_gross = float(pretax_bal)
        if _pt_gross > 0:
            _liq_rows.append(("Pre-Tax IRA/401k", _pt_gross, _pt_gross))

        _hsa_gross = float(D["curr_hsa"])
        if _hsa_gross > 0:
            _liq_rows.append(("HSA", _hsa_gross, _hsa_gross))

        _inh_ira_gross = float(D.get("inh_ira_f", 0)) + float(D.get("inh_ira_s", 0))
        if _inh_ira_gross > 0:
            _liq_rows.append(("Inherited IRA", _inh_ira_gross, _inh_ira_gross))

        _roth_gross = float(roth_bal)
        if _roth_gross > 0:
            _liq_rows.append(("Roth IRA", _roth_gross, 0.0))

        _ann_gross = float(annuity_value)
        _ann_taxable = max(0.0, _ann_gross - float(annuity_basis))
        if _ann_gross > 0:
            _liq_rows.append(("Annuity", _ann_gross, _ann_taxable))

        _life_gross = float(life_cash_value)
        _life_taxable = max(0.0, _life_gross - float(D["life_basis"]))
        if _life_gross > 0:
            _liq_rows.append(("Life Insurance CV", _life_gross, _life_taxable))

        _home_equity = max(0.0, float(home_value) - float(mortgage_balance))
        if _home_equity > 0:
            _liq_rows.append(("Home Equity (§121)", _home_equity, 0.0))

        # Investment Real Estate
        _ire_gross = 0.0; _ire_cg = 0.0
        if D["inv_re_value"] > 0:
            _ire_gross += float(D["inv_re_value"])
            _ire_cg += max(0.0, float(D["inv_re_value"]) - float(D["inv_re_basis"]))
        for _ri in range(1, 4):
            if D.get(f"inv_re_{_ri}_include", False) and D[f"inv_re_{_ri}_value"] > 0:
                _rv = float(D[f"inv_re_{_ri}_value"])
                _rm = float(D[f"inv_re_{_ri}_mortgage"])
                _ire_gross += max(0.0, _rv - _rm)
                _ire_cg += max(0.0, _rv - float(D[f"inv_re_{_ri}_basis"]))
        if _ire_gross > 0:
            _liq_rows.append(("Investment Real Estate", _ire_gross, _ire_cg))

        _liq_total_gross = sum(row[1] for row in _liq_rows)

        if _liq_total_gross > 0:
            st.divider()
            st.subheader("Net Liquidation Value")
            st.caption("Estimated after-tax value if all assets were liquidated today. "
                       "Tax includes federal, state, and IRMAA impact.")

            _liq_total_taxable = sum(row[2] for row in _liq_rows)
            if _liq_total_taxable > 0:
                _liq_inp = current_inputs()
                _liq_inp["taxable_ira"] += _pt_gross + _hsa_gross + _inh_ira_gross
                _liq_inp["cap_gain_loss"] += _brk_taxable + _conc_taxable + _ire_cg
                _liq_inp["other_income"] += _ann_taxable + _life_taxable
                _liq_result = TEA.compute_case(_liq_inp)
                _base_cost = r["total_tax"] + r.get("medicare_premiums", 0.0)
                _liq_cost = _liq_result["total_tax"] + _liq_result.get("medicare_premiums", 0.0)
                _liq_tax = max(0.0, _liq_cost - _base_cost)
            else:
                _liq_tax = 0.0

            _liq_total_net = _liq_total_gross - _liq_tax

            _md = "| Asset | Gross | Est. Tax | Net |\n|---|---:|---:|---:|\n"
            for _label, _gross, _taxable in _liq_rows:
                if _liq_total_taxable > 0 and _taxable > 0:
                    _alloc = _liq_tax * (_taxable / _liq_total_taxable)
                else:
                    _alloc = 0.0
                _md += f"| {_label} | {TEA.money(_gross)} | {TEA.money(_alloc)} | {TEA.money(_gross - _alloc)} |\n"
            _md += f"| **Total** | **{TEA.money(_liq_total_gross)}** | **{TEA.money(_liq_tax)}** | **{TEA.money(_liq_total_net)}** |\n"
            st.markdown(_md)
    ...
    if not _is_working:
        ...
        with tab2:
            ...
            if True:
                _t2_left, _t2_right = st.columns([3, 2])
                with _t2_left:
                ...
                with _t2_right:
                    _fs_label = "Joint" if is_joint else "Single"
                    # --- Federal Ordinary Brackets ---
                    _ord = [(0,24800,.10),(24800,100800,.12),(100800,211400,.22),(211400,403550,.24),(403550,512450,.32),(512450,768700,.35),(768700,None,.37)] if is_joint else \
                           [(0,12400,.10),(12400,50400,.12),(50400,105700,.22),(105700,201775,.24),(201775,256225,.32),(256225,640600,.35),(640600,None,.37)]
                    _md = "| Rate | Taxable Income |\n|---:|:---|\n"
                    for _lo, _hi, _r in _ord:
                        _md += f"| {_r:.0%} | ${_lo:,.0f}+ |\n" if _hi is None else f"| {_r:.0%} | ${_lo:,.0f} – ${_hi:,.0f} |\n"
                    st.caption(f"Federal Brackets ({_fs_label})")
                    st.markdown(_md)
                    # --- LTCG Brackets ---
                    _ltcg = TEA.get_preferential_brackets(filing_status)
                    _md = "| Rate | Taxable Income |\n|---:|:---|\n"
                    for _lo, _hi, _r in _ltcg:
                        _md += f"| {int(_r*100)}% | ${_lo:,.0f}+ |\n" if _hi == float("inf") else f"| {int(_r*100)}% | ${_lo:,.0f} – ${_hi:,.0f} |\n"
                    st.caption(f"LTCG Rates ({_fs_label})")
                    st.markdown(_md)
                    # --- IRMAA Thresholds ---
                    _irmaa = [(218000,109000,81.20,14.50),(274000,137000,202.90,37.50),(342000,171000,324.60,60.40),(410000,205000,446.30,83.30),(750000,500000,487.00,91.00)]
                    _md = "| MAGI | Surcharge |\n|---:|---:|\n"
                    for _jt, _sn, _pb, _pd in _irmaa:
                        _thresh = _jt if is_joint else _sn
                        _md += f"| >${_thresh:,.0f} | +${_pb+_pd:,.0f}/mo |\n"
                    st.caption(f"IRMAA Surcharges ({_fs_label})")
                    st.markdown(_md)
                ...
                with st.expander("Additional Expenses", expanded=len(st.session_state.additional_expenses) > 0):
                    ...
                    _ae_sources = ["Available Cash", "Taxable \u2013 Cash", "Taxable \u2013 Brokerage", "Pre-Tax \u2013 IRA/401k", "Annuity", "Roth", "Life Insurance (loan)"]
                    if is_joint:
                        _ae_sources.insert(_ae_sources.index("Roth") + 1, "Roth \u2014 Spouse")
                        _ae_sources.insert(_ae_sources.index("Annuity") + 1, "Annuity \u2014 Spouse")
                    _filer_age_now = TEA.age_at_date(filer_dob, dt.date.today()) if filer_dob else 70
                ...
                with st.expander("Future Income", expanded=len(st.session_state.future_income) > 0):
                    ...
                    for idx, fi in enumerate(st.session_state.future_income):
                        ...
                        with c6: fi["tax_type"] = st.selectbox("Tax Type", _fi_tax_types, index=_fi_tax_types.index(fi["tax_type"]), key=f"fp_fi_tax_{idx}")
                        with c7:
                            if st.button("Remove", key=f"fp_fi_rm_{idx}"):
                                _fi_to_remove = idx
                        if fi["tax_type"] == "Capital Gains":
                            fi["basis"] = st.number_input("Cost Basis $", value=float(fi["basis"]), min_value=0.0, step=1000.0, key=f"fp_fi_basis_{idx}")
                        ...
                        st.caption(f"Duration: {_fi_duration} | {fi['tax_type']}")
                        st.markdown("---")
                        ...
                        for fi in st.session_state.future_income:
                            if fi["amount"] > 0:
                                ...
                                _fi_summary.append({
                                    ...
                                    "Tax Type": fi.get("tax_type", "Ordinary Income"),
                                })
                        if _fi_summary:
                            st.markdown("#### Summary")
        ...
        with tab3:
            ...
            if st.session_state.assets is None:
                    ...
                    if _fi_active:
                        ...
                        for fi in _fi_active:
                            ...
                            _fi_disp.append({
                                ...
                                "Tax Type": fi.get("tax_type", "Ordinary Income"), "Inflates": "Yes" if fi["inflates"] else "No",
                            })
                ...
                if st.button("Run Wealth Projection", type="primary", key="fp_run_proj"):
                    ...
                    _hide_if_zero = [
                        "Mtg Pmt", "Addl Exp", "Extra Income", "Other Inc",
                        "Pension", "QCD", "Conversion", "Harvest Gains", "Surplus",
                        "Stock Gift", "Charity Bequest",
                        "W/D Cash", "W/D Taxable", "W/D Conc", "W/D Pre-Tax", "W/D Roth", "W/D Life", "W/D Annuity",
                        "Bal Cash", "Bal EF", "Bal Taxable", "Bal Conc", "Bal Pre-Tax", "Bal Roth", "Bal Annuity", "Bal Life", "Bal DB",
                    ]
                    for _col in _hide_if_zero:
                        if all(r.get(_col, 0) == 0 for r in rows):
                            _hide_cols.append(_col)
                    ...
                    if len(rows) > 0:
                        ...
                        _has_charity_bequest = result.get("charitable_bequest", 0) > 0
                        _has_stock_donated = result.get("total_stock_donated", 0) > 0
                        _n_cols = 5 + (1 if _has_estate_tax else 0) + (1 if _has_charity_bequest else 0) + (1 if _has_stock_donated else 0)
                        ...
                        if _has_estate_tax:
                            ...
                            _ci += 1
                        if _has_charity_bequest:
                            with _cols[_ci]: st.metric("Charity Bequest", f"${result['charitable_bequest']:,.0f}")
                            _ci += 1
                        with _cols[_ci]: st.metric("Net Estate", f"${rows[-1]['Estate (Net)']:,.0f}")
                        _ci += 1
                        if _has_stock_donated:
                            with _cols[_ci]: st.metric("Stock Donated", f"${result['total_stock_donated']:,.0f}")
                            _ci += 1
                        with _cols[_ci]: st.metric("Total Taxes Paid", f"${sum(r['Taxes'] + r['Medicare'] for r in rows):,.0f}")
                        with _cols[_ci+1]: st.metric("Final Year Spending", f"${rows[-1]['Spending']:,.0f}")

                        # Store tax results for 1040 display (rendered outside button block)
                        st.session_state.tab3_tax_results = result.get("tax_results", [])
                ...
                # ── 1040 Tax Return Detail (outside button block so selectbox survives reruns) ──
                _t3_tax_results = st.session_state.get("tab3_tax_results", [])
                _t3_rows = st.session_state.get("tab3_rows")
                if _t3_tax_results and _t3_rows:
                    st.divider()
                    st.markdown("### Tax Return Detail")
                    _yr_labels = [f"{r['Year']} (Age {r['Age']})" for r in _t3_rows]
                    _sel_yr_idx = st.selectbox("Select year:", range(len(_yr_labels)),
                                               format_func=lambda i: _yr_labels[i], key="fp_t3_1040_yr")
                    _sel_tr = _t3_tax_results[_sel_yr_idx]
                    with st.expander(f"1040 Return — {_yr_labels[_sel_yr_idx]}", expanded=True):
                        TEA.display_tax_return(_sel_tr,
                            filer_65=_sel_tr.get("_filer_65", False),
                            spouse_65=_sel_tr.get("_spouse_65", False))
        ...
        def _package_result(result, strat, spending_label, conv_label, scan_phase,
            ...
            return {
                ...
                "charitable_bequest": result.get("charitable_bequest", 0),
                "total_stock_donated": result.get("total_stock_donated", 0),
                ...
                "_year_details": _yd,
                "_tax_results": result.get("tax_results", []),
                "_conv_strat": conv_strat,
                "_conv_years": conv_years,
                # PDF backward compat
                "waterfall": spending_label,
                "order": strat.get("key", strat.get("type", "")),
            }

        def _make_plus_params(params, scenario):
            """Build modified params for Optimizer Plus scenarios. Total return unchanged; only yield decomposition differs."""
            import copy
            if scenario == "current":
                return params
            p = copy.deepcopy(params)
            if p.get("aa_yields") is not None:
                # Asset-allocation path — recompute taxable yields
                ty = p["aa_yields"]["taxable"]
                orig_total = ty["total"]
                if scenario == "etfs":
                    new_ty = _compute_aa_yields("taxable", vehicle_type="etfs")
                elif scenario == "individual":
                    new_ty = _compute_aa_yields("taxable", vehicle_type="individual")
                elif scenario == "asset_location":
                    # Bonds/income in IRA, stocks in taxable: zero interest, all divs qualified, recompute deferred
                    new_ty = dict(ty)
                    new_ty["int_yield"] = 0.0
                    new_ty["qual_pct"] = 1.0
                    new_ty["deferred"] = orig_total - new_ty["div_yield"] - new_ty["cg_yield"]
                else:
                    return p
                # Preserve total return
                if scenario != "asset_location":
                    new_ty["total"] = orig_total
                    new_ty["deferred"] = orig_total - new_ty["div_yield"] - new_ty["int_yield"] - new_ty["cg_yield"]
                p["aa_yields"]["taxable"] = new_ty
                # Sync legacy dollar fields for consistency
                _brok = taxable_brokerage_bal
                p["interest_taxable"] = _brok * new_ty["int_yield"]
                p["total_ordinary_dividends"] = _brok * new_ty["div_yield"]
                p["qualified_dividends"] = _brok * new_ty["div_yield"] * new_ty["qual_pct"]
                p["cap_gain_loss"] = _brok * new_ty["cg_yield"]
            else:
                # Legacy path — modify dollar amounts directly
                _brok = taxable_brokerage_bal
                if _brok < 1:
                    return p
                if scenario == "etfs":
                    # Reduce CG distributions to ETF-level (0.2% of balance)
                    p["cap_gain_loss"] = _brok * 0.002
                elif scenario == "individual":
                    # No forced CG distributions
                    p["cap_gain_loss"] = 0.0
                elif scenario == "asset_location":
                    # Bonds in IRA: zero interest, all divs qualified
                    p["interest_taxable"] = 0.0
                    _divs = p["total_ordinary_dividends"]
                    p["qualified_dividends"] = _divs  # all qualified
                else:
                    return p
            return p
        ...
        with tab4:
            ...
            if st.session_state.assets is None:
                ...
                with _btn_c1:
                    _run_opt_clicked = st.button("Run Optimizer", type="primary", key="fp_run_optimizer")
                with _btn_c2:
                    _run_plus_clicked = st.button("Run Optimizer Plus", key="fp_run_optimizer_plus",
                                                   help="Re-tests top strategies with alternative vehicle types and optimized asset location")

                if _run_opt_clicked:
                    st.session_state.opt_plus_results = None; st.session_state.opt_plus_scenarios = None
                    ...
                    for cap in _blend_caps:
                        ...
                        _spending_strategies.append({"key": "blend", "label": label, "wf": [], "blend": False, "pt_cap": cap, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    _spending_strategies.append({"key": "prorata", "label": "Pro-Rata: All Accounts (Equal Weight)", "wf": [], "blend": False, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    _spending_strategies.append({"key": "prorata_pt_heavy", "label": "Pro-Rata: Heavy Pre-Tax (2x), Light Roth (0.25x)", "wf": [], "blend": False, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    _spending_strategies.append({"key": "prorata_no_roth", "label": "Pro-Rata: All Except Roth", "wf": [], "blend": False, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    _spending_strategies.append({"key": "dynamic", "label": "Dynamic Blend (Marginal Cost)", "wf": [], "blend": True, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    _spending_strategies.append({"key": "dynamic_heir", "label": "Dynamic Blend (Heir Arbitrage)", "wf": [], "blend": True, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    ...
                    for _perm in permutations(_wf_buckets):
                        ...
                        if _wf_key not in _seen_wf:
                            ...
                            _spending_strategies.append({"key": "wf", "label": f"WF: {_wf_key}", "wf": _wf_list, "blend": False, "pt_cap": None, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                    if a0["annuity"]["value"] > 0 and a0["annuity"]["value"] > a0["annuity"]["basis"]:
                        for _depl_yrs in [5, 10, 15]:
                            _spending_strategies.append({"key": "blend", "label": f"Draw Ann Gains {_depl_yrs}yr", "wf": [], "blend": False, "pt_cap": 0, "adaptive": None, "ann_depl_yrs": _depl_yrs, "ann_gains_only": True, "conv_strat": "none", "harvest_bracket": None})
                    for _hv_accel, _hv_label in [(None, "Harvest 0% LTCG Gains"), (None, "Brokerage First + Harvest 0% Gains")]:
                        _spending_strategies.append({"key": "blend", "label": _hv_label, "wf": [], "blend": False, "pt_cap": 0, "adaptive": None, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": 0.0})

                    # Adaptive strategies (separate list)
                    _adaptive_strategies = []
                    for _akey, _adef in TEA.ADAPTIVE_STRATEGIES.items():
                        _adaptive_strategies.append({"key": "adaptive", "label": _adef["label"], "wf": [], "blend": False, "pt_cap": None, "adaptive": _akey, "ann_depl_yrs": None, "conv_strat": "none", "harvest_bracket": None})
                        ...
                        for _conv_val, _conv_label in _conv_strategies:
                            for _cyrs in _year_variants:
                                ...
                                pkg = _package_result(result, _strat, _strat["label"],
                                                      ...
                                                      conv_strat=_conv_val, conv_years=_cyrs)
                                all_results.append(pkg)
                                _run_count += 1
                                ...
                                for _ramt in _refine_amounts:
                                    for _ryrs in _refine_years:
                                        ...
                                        pkg = _package_result(result, _best_strat, _best_strat["label"],
                                                              ...
                                                              conv_strat=float(_ramt), conv_years=_ryrs)
                                        all_results.append(pkg)
                    ...
                    for r in _p2_compat:
                        _p2_mapped.append({
                            ...
                            "_tax_results": r.get("_tax_results", []),
                        })
                ...
                # ════════════════════════════════════════════
                # OPTIMIZER PLUS: Vehicle & Asset Location
                # ════════════════════════════════════════════
                if _run_plus_clicked and not st.session_state.opt_all_results:
                    st.warning("Run the Optimizer first, then click Optimizer Plus.")
                if _run_plus_clicked and st.session_state.opt_all_results:
                    _plus_params = st.session_state.opt_params
                    _plus_a0 = st.session_state.assets
                    _plus_all = st.session_state.opt_all_results

                    # Build unique estate tiers (same $500 grouping as Top 5 / MC)
                    _plus_tiers = []
                    for r in _plus_all:
                        _merged = False
                        for t in _plus_tiers:
                            if abs(t["result"]["after_tax_estate"] - r["after_tax_estate"]) < 500:
                                _merged = True; break
                        if not _merged:
                            _plus_tiers.append({"result": r, "label": f"{r['spending_label']} + {r['conversion_label']}"})
                        if len(_plus_tiers) >= 5: break

                    # Build scenario list based on current vehicle type
                    _curr_vt = D["vt_taxable"]
                    _plus_scenarios = [("current", "Current")]
                    if _curr_vt != "etfs":
                        _plus_scenarios.append(("etfs", "Individual Securities (ETFs)"))
                    if _curr_vt != "individual":
                        _plus_scenarios.append(("individual", "Individual Securities (Direct)"))
                    _plus_scenarios.append(("asset_location", "Asset Location"))
                    st.session_state.opt_plus_scenarios = _plus_scenarios

                    _plus_results = []
                    _plus_n_total = len(_plus_tiers) * len(_plus_scenarios)
                    _plus_progress = st.progress(0, text="Running Optimizer Plus...")
                    _plus_count = 0

                    for _pt in _plus_tiers:
                        _pr = _pt["result"]
                        # Reconstruct strat dict from stored result
                        _si = _pr["_strat_info"]
                        _plus_strat = {
                            "key": _si["type"], "label": _pr["spending_label"],
                            "wf": _si["wf"], "blend": _si["blend"],
                            "pt_cap": _si["pt_cap"], "adaptive": _si["adaptive_key"],
                            "ann_depl_yrs": _si["ann_depl_yrs"],
                            "ann_gains_only": _si["ann_gains_only"],
                            "harvest_bracket": _si["harvest_bracket"],
                        }
                        _plus_conv = _pr.get("_conv_strat", "none")
                        _plus_cyrs = _pr.get("_conv_years", 0)
                        _tier_row = {"strategy": _pt["label"], "tier_estate": _pr["after_tax_estate"]}

                        for _sc_key, _sc_label in _plus_scenarios:
                            _plus_count += 1
                            _plus_progress.progress(_plus_count / _plus_n_total,
                                                     text=f"Optimizer Plus ({_plus_count}/{_plus_n_total}): {_pt['label']} — {_sc_label}")
                            _mod_params = _make_plus_params(_plus_params, _sc_key)
                            _sc_result = _run_single_strategy(_plus_a0, _mod_params, _plus_strat,
                                                               conv_strat=_plus_conv, stop_age=opt_stop_age,
                                                               target_agi=target_agi_input, conv_years=_plus_cyrs,
                                                               agi_cap=agi_cap_enabled)
                            _tier_row[_sc_key] = _sc_result["after_tax_estate"]
                            _tier_row[f"{_sc_key}_taxes"] = _sc_result["total_taxes"]
                            _tier_row[f"{_sc_key}_inv_inc"] = sum(r.get("Inv Inc", 0) for r in _sc_result["year_details"])
                            _tier_row[f"{_sc_key}_details"] = _sc_result["year_details"]
                            _tier_row[f"{_sc_key}_tax_results"] = _sc_result.get("tax_results", [])

                        # Compute best scenario and delta for this tier
                        _sc_estates = {k: _tier_row[k] for k, _ in _plus_scenarios}
                        _best_sc = max(_sc_estates, key=_sc_estates.get)
                        _tier_row["best_scenario"] = _best_sc
                        _tier_row["best_label"] = dict(_plus_scenarios)[_best_sc]
                        _tier_row["delta"] = _sc_estates[_best_sc] - _tier_row["current"]
                        _plus_results.append(_tier_row)

                    _plus_progress.empty()
                    st.session_state.opt_plus_results = _plus_results
                    st.success(f"Optimizer Plus: tested {_plus_count} scenarios across {len(_plus_tiers)} strategies")
                ...
                if st.session_state.opt_all_results:
                    ...
                    # ---- 1040 Tax Return Detail ----
                    _opt_tax_results = _selected_combo.get("_tax_results", [])
                    if _opt_tax_results and _sel_details:
                        st.divider()
                        st.markdown("### Tax Return Detail")
                        _yr_labels_opt = [f"{r['Year']} (Age {r['Age']})" for r in _sel_details]
                        _sel_yr_opt = st.selectbox("Select year:", range(len(_yr_labels_opt)),
                                                   format_func=lambda i: _yr_labels_opt[i], key="fp_opt_1040_yr")
                        _sel_tr_opt = _opt_tax_results[_sel_yr_opt]
                        with st.expander(f"1040 Return — {_yr_labels_opt[_sel_yr_opt]}", expanded=True):
                            TEA.display_tax_return(_sel_tr_opt,
                                filer_65=_sel_tr_opt.get("_filer_65", False),
                                spouse_65=_sel_tr_opt.get("_spouse_65", False))

                    # ---- Side-by-side: Wealth Projection baseline vs selected combo ----
                    _bl_details = st.session_state.get("tab3_rows", [])
                    if _bl_details and _sel_details:
                        st.divider()
                        yr1_bl = _bl_details[0] if _bl_details else {}; yr1_bc = _sel_details[0] if _sel_details else {}
                        st.markdown(f"### Year 1 Cash Flow: Wealth Projection vs {_selected_combo_label}")
                        col1, col2 = st.columns(2)
                        for col, yr, label in [(col1, yr1_bl, "Wealth Projection"), (col2, yr1_bc, _selected_combo_label)]:
                            with col:
                                st.markdown(f"#### {label}")
                    ...
                    # ════════════════════════════════════════════
                    # OPTIMIZER PLUS RESULTS DISPLAY
                    # ════════════════════════════════════════════
                    if st.session_state.opt_plus_results:
                        _opr = st.session_state.opt_plus_results
                        _ops = st.session_state.opt_plus_scenarios or []
                        st.divider()
                        st.markdown("### Optimizer Plus: Vehicle & Location Analysis")

                        # ---- Flatten combos ----
                        _plus_combos = []
                        _plus_baselines = {}  # strategy_label -> combo dict for "current" scenario
                        for _pr in _opr:
                            for _sk, _sl in _ops:
                                _combo = {
                                    "combo_label": f"{_pr['strategy']} / {_sl}",
                                    "strategy": _pr["strategy"],
                                    "scenario_key": _sk,
                                    "scenario_label": _sl,
                                    "after_tax_estate": _pr[_sk],
                                    "total_taxes": _pr.get(f"{_sk}_taxes", 0),
                                    "year_details": _pr.get(f"{_sk}_details", []),
                                    "tax_results": _pr.get(f"{_sk}_tax_results", []),
                                }
                                _plus_combos.append(_combo)
                                if _sk == "current":
                                    _plus_baselines[_pr["strategy"]] = _combo
                        _plus_combos.sort(key=lambda x: x["after_tax_estate"], reverse=True)
                        _plus_best = _plus_combos[0]

                        # ---- Best highlight ----
                        _plus_bl = _plus_baselines.get(_plus_best["strategy"])
                        _plus_improvement = _plus_best["after_tax_estate"] - _plus_bl["after_tax_estate"] if _plus_bl else 0
                        if _plus_improvement > 100:
                            st.success(f"**Best:** {_plus_best['combo_label']} — Estate: **${_plus_best['after_tax_estate']:,.0f}** "
                                       f"(+${_plus_improvement:,.0f} vs Current)")
                        else:
                            st.success(f"**Best:** {_plus_best['combo_label']} — Estate: **${_plus_best['after_tax_estate']:,.0f}**")

                        # ---- Summary metrics ----
                        _pm_cols = st.columns(3)
                        with _pm_cols[0]: st.metric("Best Net Estate", f"${_plus_best['after_tax_estate']:,.0f}")
                        with _pm_cols[1]: st.metric("Best Scenario", _plus_best["scenario_label"])
                        with _pm_cols[2]: st.metric("vs Current", f"${_plus_improvement:+,.0f}" if abs(_plus_improvement) > 100 else "$0")

                        # ---- Estate comparison table ----
                        _plus_tbl_rows = []
                        for _pr in _opr:
                            _row = {"Strategy": _pr["strategy"]}
                            for _sk, _sl in _ops:
                                _row[_sl] = f"${_pr[_sk]:,.0f}"
                            _row["Best"] = _pr["best_label"]
                            _row["Delta"] = f"${_pr['delta']:+,.0f}" if abs(_pr["delta"]) > 100 else "$0"
                            _plus_tbl_rows.append(_row)
                        st.dataframe(pd.DataFrame(_plus_tbl_rows), use_container_width=True, hide_index=True)

                        # ---- Combo selector ----
                        _plus_combo_labels = [c["combo_label"] for c in _plus_combos]
                        _plus_sel_label = st.selectbox("Select combination to view detail", _plus_combo_labels, index=0, key="fp_plus_combo_select")
                        _plus_sel_idx = _plus_combo_labels.index(_plus_sel_label)
                        _plus_sel = _plus_combos[_plus_sel_idx]

                        if _plus_sel_label != _plus_best["combo_label"]:
                            _pc1, _pc2, _pc3 = st.columns(3)
                            with _pc1: st.metric("Selected Net Estate", f"${_plus_sel['after_tax_estate']:,.0f}")
                            with _pc2: st.metric("Selected Scenario", _plus_sel["scenario_label"])
                            with _pc3: st.metric("vs Best", f"${_plus_sel['after_tax_estate'] - _plus_best['after_tax_estate']:+,.0f}")

                        # ---- Year-by-Year Detail ----
                        _plus_sel_details = _plus_sel.get("year_details", [])
                        if _plus_sel_details:
                            with st.expander(f"Year-by-Year Detail: {_plus_sel_label}", expanded=True):
                                _df_pdet = pd.DataFrame(_plus_sel_details)
                                _plus_always_hide = ["Total Wealth", "_net_draw"]
                                _plus_zero_hide = [
                                    "Mtg Pmt", "Addl Exp", "Extra Income", "Other Inc",
                                    "Pension", "QCD", "Conversion", "Harvest Gains", "Surplus",
                                    "W/D Cash", "W/D Taxable", "W/D Conc", "W/D Pre-Tax", "W/D Roth", "W/D Life", "W/D Annuity",
                                ...
                                _df_pdet = _df_pdet.drop(columns=[c for c in _plus_always_hide if c in _df_pdet.columns], errors="ignore")
                                for _zc in _plus_zero_hide:
                                    if _zc in _df_pdet.columns and (_df_pdet[_zc] == 0).all():
                                        _df_pdet = _df_pdet.drop(columns=[_zc])
                                st.dataframe(_df_pdet, use_container_width=True, hide_index=True)

                        # ---- 1040 Tax Return Detail ----
                        _plus_tax_results = _plus_sel.get("tax_results", [])
                        if _plus_tax_results and _plus_sel_details:
                            st.divider()
                            st.markdown("### Tax Return Detail")
                            _plus_yr_labels = [f"{r['Year']} (Age {r['Age']})" for r in _plus_sel_details]
                            _plus_sel_yr = st.selectbox("Select year:", range(len(_plus_yr_labels)),
                                                         format_func=lambda i: _plus_yr_labels[i], key="fp_plus_1040_yr")
                            _plus_sel_tr = _plus_tax_results[_plus_sel_yr]
                            with st.expander(f"1040 Return — {_plus_yr_labels[_plus_sel_yr]}", expanded=True):
                                TEA.display_tax_return(_plus_sel_tr,
                                    filer_65=_plus_sel_tr.get("_filer_65", False),
                                    spouse_65=_plus_sel_tr.get("_spouse_65", False))

                        # ---- Year 1 Cash Flow: Current vs Selected ----
                        _plus_bl_combo = _plus_baselines.get(_plus_sel["strategy"])
                        _plus_bl_details = _plus_bl_combo.get("year_details", []) if _plus_bl_combo else []
                        if _plus_bl_details and _plus_sel_details:
                            st.divider()
                            _p_yr1_bl = _plus_bl_details[0] if _plus_bl_details else {}
                            _p_yr1_sel = _plus_sel_details[0] if _plus_sel_details else {}
                            st.markdown(f"### Year 1 Cash Flow: Current vs {_plus_sel['scenario_label']}")
                            _pcf1, _pcf2 = st.columns(2)
                            for _pcol, _pyr, _plbl in [(_pcf1, _p_yr1_bl, f"{_plus_sel['strategy']} (Current)"),
                                                        (_pcf2, _p_yr1_sel, _plus_sel_label)]:
                                with _pcol:
                                    st.markdown(f"#### {_plbl}")
                                    st.markdown("**Income (Cash Received)**")
                                    _p_inc_rows = [("Social Security", _pyr.get("SS", 0)), ("Pensions", _pyr.get("Pension", 0)),
                                                   ("RMD", _pyr.get("RMD", 0)), ("Investment Income", _pyr.get("Inv Inc", 0)),
                                                   ("W/D Cash", _pyr.get("W/D Cash", 0)), ("W/D Taxable", _pyr.get("W/D Taxable", 0)),
                                                   ("W/D Pre-Tax", _pyr.get("W/D Pre-Tax", 0)), ("Roth Conversion", _pyr.get("Conversion", 0)),
                                                   ("W/D Roth", _pyr.get("W/D Roth", 0)), ("W/D Life Ins", _pyr.get("W/D Life", 0)),
                                                   ("W/D Annuity", _pyr.get("W/D Annuity", 0))]
                                    _p_inc_rows = [(k, v) for k, v in _p_inc_rows if v > 0]
                                    _p_total_inc = sum(v for _, v in _p_inc_rows)
                                    st.dataframe([{"Income": k, "Amount": TEA.money(v)} for k, v in _p_inc_rows], use_container_width=True, hide_index=True)
                                    st.metric("Total Income", TEA.money(_p_total_inc))
                                    st.markdown("**Expenses & Outflows**")
                                    _p_out_rows = [("Living Expenses", _pyr.get("Spending", 0)), ("Taxes", _pyr.get("Taxes", 0)),
                                                   ("Medicare", _pyr.get("Medicare", 0))]
                                    _p_out_rows = [(k, v) for k, v in _p_out_rows if v > 0]
                                    _p_total_out = sum(v for _, v in _p_out_rows)
                                    st.dataframe([{"Expense": k, "Amount": TEA.money(v)} for k, v in _p_out_rows], use_container_width=True, hide_index=True)
                                    st.metric("Total Outflows", TEA.money(_p_total_out))

                            # ---- End of Projection Comparison ----
                            st.divider()
                            st.markdown("### End of Projection Comparison")
                            _pep1, _pep2 = st.columns(2)
                            _p_final_bl = _plus_bl_details[-1] if _plus_bl_details else {}
                            _p_final_sel = _plus_sel_details[-1] if _plus_sel_details else {}
                            with _pep1:
                                st.markdown(f"#### {_plus_sel['strategy']} (Current)")
                                st.metric("Final Pre-Tax", TEA.money(_p_final_bl.get("Bal Pre-Tax", 0)))
                                st.metric("Final Roth", TEA.money(_p_final_bl.get("Bal Roth", 0)))
                                st.metric("Final Taxable", TEA.money(_p_final_bl.get("Bal Cash", 0) + _p_final_bl.get("Bal Taxable", 0)))
                                st.metric("After-Tax Estate", TEA.money(_p_final_bl.get("Estate (Net)", 0)))
                            with _pep2:
                                st.markdown(f"#### {_plus_sel_label}")
                                st.metric("Final Pre-Tax", TEA.money(_p_final_sel.get("Bal Pre-Tax", 0)))
                                st.metric("Final Roth", TEA.money(_p_final_sel.get("Bal Roth", 0)))
                                st.metric("Final Taxable", TEA.money(_p_final_sel.get("Bal Cash", 0) + _p_final_sel.get("Bal Taxable", 0)))
                                st.metric("After-Tax Estate", TEA.money(_p_final_sel.get("Estate (Net)", 0)))
            ...
            if st.session_state.tab5_conv_res is not None:
                ...
                if _is_working:
                    ...
                    if _accum is None:
                        ...
                        _ei_params["spending_goal"] = st.session_state.get("_fp_proj_spending", D["living_expenses"]) * (D["ret_pct"] / 100) * _inf_factor
...
# ══════════════════════════════════════════════════════════════════════
# DEFERRED SAVE — runs after all UI code has updated lists
# ══════════════════════════════════════════════════════════════════════
